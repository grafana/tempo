...
dashboard_utils {
  ...
  local limitQuery(limit) = |||
    max(
      ...
      or max by (cluster, namespace, limit_name) (tempo_limits_defaults{%(matcher)s,user="$tenant",limit_name="%(limit_name)s"})
    ) by (%(limit_name)s)
  ||| % {
    limit_name: limit,
    matcher: $.jobMatcher($._config.jobs.distributor),  // TODO(mapno): use the correct job for each limit
  },
    ...
    'tempo-tenants.json':
            ...
            [
              |||
                max(
                  ...
                  or max by (cluster, namespace, limit_name) (tempo_limits_defaults{%s,user="$tenant"})
                ) by (limit_name)
              ||| % [$.jobMatcher($._config.jobs.distributor), $.jobMatcher($._config.jobs.distributor)],
            ], {}
          ),
        )
      )
      .addRow(
        ...
        .addPanel(
          ...
          $.queryPanel(
            [
              'sum(rate(tempo_distributor_bytes_received_total{%s,tenant="$tenant"}[$__rate_interval])) by (cluster, namespace, tenant)' % $.jobMatcher($._config.jobs.distributor),
        ...
        .addPanel(
          ...
          $.queryPanel(
            [
              ...
              'sum(rate(tempo_discarded_spans_total{%s,tenant=~"$tenant"}[$__rate_interval])) by (cluster, namespace, reason)' % $.jobMatcher($._config.jobs.distributor),
            ],
            [
              'accepted',
              'refused {{ reason }}',
        ...
        .addPanel(
          ...
          $.queryPanel(
            [
              'max(tempo_ingester_live_traces{cluster="$cluster",namespace="$namespace",tenant="$tenant"})',
