...
func TestScalableSingleBinary(t *testing.T) {
	...
	c1, err := util.NewOtlpJaegerClient(tempo1.Endpoint(14250))
	require.NoError(t, err)
	require.NotNil(t, c1)

	c2, err := util.NewOtlpJaegerClient(tempo2.Endpoint(14250))
	require.NoError(t, err)
	require.NotNil(t, c2)

	c3, err := util.NewOtlpJaegerClient(tempo3.Endpoint(14250))
	require.NoError(t, err)
	require.NotNil(t, c3)
