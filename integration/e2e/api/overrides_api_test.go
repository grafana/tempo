...
func setupOverridesAPITest(t *testing.T) (*e2e.Scenario, *e2e.HTTPService) {
	s, err := e2e.NewScenario("tempo_e2e")
	require.NoError(t, err)

	// copy config template to shared directory and expand template variables
	tmplConfig := map[string]any{"Prefix": ""}
	configFile, err := util.CopyTemplateToSharedDir(s, configAllInOneS3Overrides, "config.yaml", tmplConfig)
	require.NoError(t, err)
...
func TestOverridesAPI_GET(t *testing.T) {
	s, tempo := setupOverridesAPITest(t)
	defer s.Close()
