...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::File::open(&tempo_proto_path)?
        .read_to_string(&mut tempo_proto_content)?;
    ...
    tempo_proto_content = tempo_proto_content
        .replace("import \"common/v1/common.proto\"", "import \"opentelemetry/proto/common/v1/common.proto\"")
        .replace("import \"trace/v1/trace.proto\"", "import \"opentelemetry/proto/trace/v1/trace.proto\"")
        .replace("tempopb.common.v1.KeyValue", "opentelemetry.proto.common.v1.KeyValue")
        .replace("tempopb.trace.v1.ResourceSpans", "opentelemetry.proto.trace.v1.ResourceSpans")
        .replace("tempopb.trace.v1.Span.Link", "opentelemetry.proto.trace.v1.Span.Link");

    // Write adapted proto to a temporary file
    let out_dir = PathBuf::from(std::env::var("OUT_DIR")?);
    let adapted_proto_path = out_dir.join("tempo_adapted.proto");
    fs::File::create(&adapted_proto_path)?
        .write_all(tempo_proto_content.as_bytes())?;
