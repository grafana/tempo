...
filtered_resources AS (
  ...
  WHERE resource."Resource"."K8sClusterName" ~ 'prod.*' AND resource."Resource"."K8sNamespaceName" = 'hosted-grafana' AND resource."Resource"."K8sContainerName" = 'hosted-grafana-gateway'
),
...
filtered_spans AS (
  ...
  WHERE span."Name" = 'httpclient/grafana' AND span."HttpStatusCode" = 200 AND span."DurationNano" > 20000000
)
