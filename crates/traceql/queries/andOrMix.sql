...
filtered_spans AS (
  ...
  WHERE list_contains(flatten(map_extract(attrs_to_map(span."Attrs"), 'foo')), 'a') AND list_contains(flatten(map_extract(attrs_to_map(span."Attrs"), 'bar')), 'a') AND list_contains(flatten(map_extract(attrs_to_map(span."Attrs"), 'foo')), 'b') AND NOT list_contains(flatten(map_extract(attrs_to_map(span."Attrs"), 'bar')), 'a') AND NOT list_contains(list_contains(list_contains(list_contains(((flatten(map_extract(attrs_to_map(span."Attrs"), 'foo')), 'a') AND flatten(map_extract(attrs_to_map(span."Attrs"), 'bar')), 'a')) OR (flatten(map_extract(attrs_to_map(span."Attrs"), 'foo')), 'b') AND flatten(map_extract(attrs_to_map(span."Attrs"), 'bar')), 'a')))
)
