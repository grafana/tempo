...
filtered_resources AS (
  ...
  WHERE list_contains(flatten(map_extract(attrs_to_map(resource."Resource"."Attrs"), 'foo')), 'bar') AND list_contains((flatten(map_extract(attrs_to_map(resource."Resource"."Attrs"), 'foo')), 'bar') OR span."Name" = 'gcs.ReadRange')
),
