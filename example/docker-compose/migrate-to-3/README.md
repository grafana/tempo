...
- **v3 containers fail with unknown target errors** → Ensure
  `docker-compose.yaml`'s `x-tempo-v3-image` anchor is pinned to
  `grafana/tempo:3.0.0` or newer. Older 2.x images don't include 3.0
  components.
