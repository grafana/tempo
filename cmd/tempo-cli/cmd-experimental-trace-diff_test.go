...
func TestExperimentalTraceDiffWritesTraceSummary(t *testing.T) {
	...
	cmd := experimentalTraceDiffCmd{
		TraceA: traceAFile,
		TraceB: traceBFile,
		Format: tracediff.VersionTraceSummaryV0Ranked,
		Out:    outFile,
	}
	require.NoError(t, cmd.Run(nil))
	...
	require.NoError(t, json.Unmarshal(bytes, &result))
	require.Equal(t, tracediff.VersionTraceSummaryV0Ranked, result.Version)
	...
	require.NotNil(t, result.TopChanges)
	require.Empty(t, result.Groups)
}

func TestExperimentalTraceDiffWritesTraceSummaryGrouped(t *testing.T) {
	dir := t.TempDir()
	traceAFile := filepath.Join(dir, "trace-a.json")
	traceBFile := filepath.Join(dir, "trace-b.json")
	outFile := filepath.Join(dir, "summary.json")
	...
	cmd := experimentalTraceDiffCmd{
		TraceA: traceAFile,
		TraceB: traceBFile,
		Format: tracediff.VersionTraceSummaryV0Grouped,
		Out:    outFile,
	}
	require.NoError(t, cmd.Run(nil))
	...
	var result tracediff.SummaryResult
	require.NoError(t, json.Unmarshal(bytes, &result))
	require.Equal(t, tracediff.VersionTraceSummaryV0Grouped, result.Version)
	require.Nil(t, result.TopChanges)
...
func TestExperimentalTraceDiffSummaryWarnsOnPartialTraceFile(t *testing.T) {
	...
	cmd := experimentalTraceDiffCmd{
		TraceA: traceAFile,
		TraceB: traceBFile,
		Format: tracediff.VersionTraceSummaryV0Aggregate,
		Out:    outFile,
	}
	require.NoError(t, cmd.Run(nil))
	...
	var result tracediff.SummaryResult
	require.NoError(t, json.Unmarshal(bytes, &result))
	require.Equal(t, tracediff.VersionTraceSummaryV0Aggregate, result.Version)
	require.Nil(t, result.TopChanges)
	require.Empty(t, result.Groups)
