...
* [FEATURE] metrics-generator: add service-graphs subprocessors that emit subsets of the metrics produced by the `service-graphs` processor. New processor names: `service-graphs-request`, `service-graphs-latency`, and `service-graphs-connection-info`. The new `traces_service_graph_connection_info` gauge is an info-style presence signal (held at 1 while the edge is being observed) intended for long-window range queries that reliably detect service-to-service relationships under aggressive head sampling. Connection-info is opt-in and is not enabled by the bare `service-graphs` name. [#7202](https://github.com/grafana/tempo/pull/7202) (@jcreixell)
