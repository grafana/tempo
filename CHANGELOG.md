...
* [FEATURE] querier: Add per-row-group cache for TraceQL metric query partial results. When the new `metrics-query-row-group` cache role is configured, the querier caches one `*tempopb.QueryRangeResponse` per (tenant, block UUID, row group index, canonicalized query). Cache hits skip TraceQL engine work for already-evaluated row groups. vParquet5 only.
