...
- `tempo-vulture`: keyless-sign published images with cosign and attach SLSA build provenance. ([#7493](https://github.com/grafana/tempo/issues/7493)) (@mattdurham)
...
- `querier`: Instrument querier methods with new stats metrics ([#7571](https://github.com/grafana/tempo/issues/7571)) (@javiermolinar)
- `query-frontend`: Implements the diff endpoint. No cache or extra protection mechanisms yet ([#7539](https://github.com/grafana/tempo/issues/7539)) (@javiermolinar)
- `query-frontend`: Added span pruning support to the trace-by-id v2 endpoint. ([#7566](https://github.com/grafana/tempo/issues/7566)) (@ie-pham)
- `query-frontend`: Added a new endpoint to calculate the diff between two traces. Marked as experimental ([#7523](https://github.com/grafana/tempo/issues/7523)) (@javiermolianr)
- `query-frontend`: Added an experimental `span_pruning_enabled_by_default` option to enable span pruning by default for trace-by-id v2 requests that don't specify `span_pruning`, and detection of traces already pruned on the write path to avoid re-pruning them. ([#7628](https://github.com/grafana/tempo/issues/7628)) (@ie-pham)
...
- `backend-scheduler`: add `tempo_backend_scheduler_jobs_pending` and complete native-histogram coverage. ([#7772](https://github.com/grafana/tempo/issues/7772)) (@zalegrala)
...
- `block-builder`: Add `tempo_block_builder_flush_size_bytes` native histogram recording the size of blocks flushed by the block-builder. ([#7773](https://github.com/grafana/tempo/issues/7773)) (@zalegrala)
  ...
  metric, informing `max_input_blocks` tuning and TCO analysis. Paired with a
  `tempodb_compaction_output_block_size_bytes` metric added in the same PR for compaction output blocks
  (see the sibling `.chloggen` entry).
...
- `cache`: add `Remove` method to the `Cache` interface, implemented for memcached and redis. ([#7204](https://github.com/grafana/tempo/issues/7204)) (@zalegrala)
...
- `operations`: Sign and attest the tempo-query and tempo-cli container images (in addition to tempo-vulture). The main tempo image is not yet signed. ([#7543](https://github.com/grafana/tempo/issues/7543)) (@mattdurham)
- `operations`: bump `memcached` to `1.6.42-alpine` and `prom/memcached-exporter` to `v0.16.0` to clear accumulated CVEs. ([#7244](https://github.com/grafana/tempo/issues/7244)) (@zhxiaogg)
- `operations`: Fix GAR authentication for tempo-vulture attestation push and add missing attestations permission in release-tag workflow. ([#7534](https://github.com/grafana/tempo/issues/7534)) (@mattdurham)
...
- `operations`: Sign and attest the main tempo container image, completing signing coverage across all published images (tempo, tempo-vulture, tempo-query, tempo-cli). ([#7601](https://github.com/grafana/tempo/issues/7601)) (@mattdurham)
...
- `operations`: tempo-mixin: add a Latency metrics variable to the Backend Work dashboard to switch its histogram panels between classic and native. ([#7795](https://github.com/grafana/tempo/issues/7795)) (@zalegrala)
...
- `operations`: tempo-mixin: add a pending-redaction-jobs panel and native-histogram queries to the Backend Work dashboard. ([#7772](https://github.com/grafana/tempo/issues/7772)) (@zalegrala)
...
- `operations`: Reorder the Backend Work mixin dashboard variables so Namespace is selected before Cluster and filters the Cluster query, instead of the two being independent. ([#7758](https://github.com/grafana/tempo/issues/7758)) (@zalegrala)
- `operations`: update backendwork dashboard with a Redaction section (active jobs, created/completed/failed/dropped by tenant, job duration), add Dropped and Job Duration panels to the Jobs row, and fix the Retry metric name (`jobs_retry` -> `jobs_retry_total`). ([#7184](https://github.com/grafana/tempo/issues/7184)) (@zalegrala)
...
- `querier`: Merge new stats metrics added in 7504 into missing querier methods ([#7568](https://github.com/grafana/tempo/issues/7568)) (@javiermolinar)
...
- `query-frontend`: Added hard input protection for the trace diff handler, the combined size of both traces must not exceed MaxBytesPerTrace. ([#7564](https://github.com/grafana/tempo/issues/7564)) (@javiermolinar)
...
- `query-frontend`: Add per-tenant override (span_pruning_enabled) for the query-frontend's span_pruning_enabled_by_default config ([#7693](https://github.com/grafana/tempo/issues/7693)) (@ie-pham)
...
- `backend-scheduler`: fix cross-tenant escalation in SubmitRedaction -- tenant is now sourced exclusively from the authenticated request context (X-Scope-OrgID header) and the body tenant_id field is ignored. ([#7153](https://github.com/grafana/tempo/issues/7153)) (@zalegrala)
...
- `distributor`: Return a retryable status for transient errors writting into Kafka ([#7506](https://github.com/grafana/tempo/issues/7506)) (@javiermolinar)
...
- `operations`: Fix GAR authentication in the container image signing workflow so the cosign signature and SLSA provenance attestation are actually produced. The previous auth step failed on every main push, so signed/attested images were never published. ([#7565](https://github.com/grafana/tempo/issues/7565)) (@mattdurham)
...
- `deps`: Align Tempo image and release builds with Go 1.27.1 and refresh the CI tools image. ([#7913](https://github.com/grafana/tempo/issues/7913)) (@javiermolinar)
