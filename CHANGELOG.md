...
* [CHANGE] **BREAKING CHANGE** Config-file environment variable expansion (`-config.expand-env=true`) now only supports only `$VAR` and `${VAR}`. POSIX shell expansions such as `${VAR:-default}` are no longer supported. [#7218](https://github.com/grafana/tempo/issues/7218)
