...
* [ENHANCEMENT] blocklist: fix O(N²) slice scans in `updateInternal`; replace `slices.ContainsFunc` loops with pre-built map lookups, reducing per-poll cost from O(N·M) to O(N+M). [#7140](https://github.com/grafana/tempo/pull/7140) (@zalegrala)
