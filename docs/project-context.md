...
<!-- The Tempo-related charts are community-maintained and live in a **separate** repository from Tempo. -->

- **Helm chart repository:** `https://github.com/grafana-community/helm-charts` (community-maintained; source of truth for chart values). Local checkout is typically a sibling directory: `../helm-charts`
- **Tempo-related charts in that repo:** `charts/tempo`, `charts/tempo-distributed`, and `charts/tempo-vulture`. **Only `tempo-distributed` is documented** in the Tempo docs; `tempo` (single-binary) and `tempo-vulture` are not
- **Documented chart path in that repo:** `charts/tempo-distributed`
- **Helm docs location (filesystem):** `docs/sources/helm-charts/tempo-distributed/` (in this repo — prose install/get-started/release-notes pages)
- **Generated Helm values reference (do not hand-edit):** `charts/tempo-distributed/README.md` in the helm-charts repo (generated by helm-docs from `values.yaml`). There is no generated values reference inside the Tempo docs tree
...
- **Code-adjacent `AGENTS.md` files:** check for an `AGENTS.md` in the relevant directory before docs work that touches that area — repo root `AGENTS.md`, `modules/generator/AGENTS.md` (metrics-generator), and `docs/sources/tempo/configuration/AGENTS.md` (notes on the auto-generated config `manifest.md`)
