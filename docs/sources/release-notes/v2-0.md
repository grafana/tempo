...
To learn more about the TraceQL syntax, see the TraceQL documentation. For information on how we plan to extend the TraceQL language, see future work.
...
In our own environments, we’re seeing that it allows us to scan trace data at rates of 1 TB/s (with some bursts as high as 4 TB/s), a massive improvement over the **XXX** rates that were more typical with our previous storage format. Switching to the Parquet block format also allows users to leverage the large ecosystem of existing Parquet tools and libraries to process and transform their trace data.
...
>**NOTE:** The metrics-generator is not turned on by default for Grafana Cloud customers due to the potential cost implications.
