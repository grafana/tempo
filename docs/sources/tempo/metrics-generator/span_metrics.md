...
{{< admonition type="note" >}}
In Tempo 1.4 and 1.4.1, the histogram metric was called `traces_spanmetrics_duration_seconds`. This was changed later to be consistent with the metrics generated by Grafana Agent and the OpenTelemetry Collector.
{{% /admonition %}}
...
If you use a ratio-based sampler, you can use the custom sampler below to not lose metric information. However, you also need to set `metrics_generator.processor.span_metrics.multiplier_key` to `"X-SampleRatio"`.
