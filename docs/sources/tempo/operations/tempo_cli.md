...
`attr != nil`, `attr != ""` and `attr > ""` select spans that **have** the attribute: the first where it is present, the other two where it is present and non-empty.
They are not negations or orderings. TraceQL resolves a missing attribute to nil, treats a nil comparison as no match, and refuses ordered comparisons against nil outright, so none of the three selects the spans that lack the attribute.
`!=` against any other value is refused, because that match set is everything except the value you named. So are `>=` and `<=` against `""`, which are bounded the same way but are not existence checks.

These are broader than they look on a large tenant. `span.user_id != ""` selects every span carrying a user ID, so preview with `--dry-run` before running one for real.
