...
Tempo maintains knowledge of the state of the backend by polling it on regular intervals. There are
a few components that need this knowledge: schedulers, workers, queriers, and query-frontends.
...
The remaining components only read the tenant index. They fall back to a full poll if the index is too far out of date or if reading the index fails.
...
- `tempodb_blocklist_tenant_index_errors_total`
  A holistic metric that increments for any error building the tenant index. Any increase in this metric should be reviewed.
