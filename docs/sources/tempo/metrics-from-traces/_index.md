...
| **Active series limits** | When the per-tenant active series limit is reached, the metrics-generator silently drops new metric series or routes them to overflow buckets. There's no customer-visible error. Metrics become incomplete without any alert. | Query `tempo_metrics_generator_registry_series_limited_total` to detect drops. Use `tempo_metrics_generator_registry_active_series_demand_estimate` to see true demand. Refer to [Max active series](/docs/tempo/<TEMPO_VERSION>/troubleshooting/metrics-generator/#max-active-series). |
