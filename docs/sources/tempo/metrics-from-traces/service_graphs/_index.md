...
| `traces_service_graph_connection_info`                  | Gauge     | client, server, connection_type | (Off by default) Info-style presence signal: held at 1 while the edge is being observed. Query with a long range window (for example `last_over_time(traces_service_graph_connection_info[1h]) > 0`) to reliably detect low-traffic relationships under aggressive head sampling. The series is dropped automatically once edges stop completing for a given label set. |
