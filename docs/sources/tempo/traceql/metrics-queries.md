...
Span metrics from the metrics-generator creating RED metrics.
TraceQL metrics can find the same RED metrics using ...

Each function has an example for how to translate a PromQL query to a TraceQL query.

### How to translate your PromQL queries into TraceQL queries using the TraceQL functions


Reference Intro to MLTP dashboard that has PromQL queries - as existing PromQL queries.
...
`count_over_time`
: Counts the number of matching spans per time interval (see the [`step` API parameter](https://grafana.com/docs/tempo/<TEMPO_VERSION>/api_docs/#traceql-metrics))
...
The following query shows the rate of errors by service and span name. This is a TraceQL specific way of gathering rate metrics that would otherwise be generated by the span metrics processor. For example:
