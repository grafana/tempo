...
To ensure fair resource usage and to prevent the "noisy neighbor" problem in multi-tenant environments, Tempo limits the number of jobs a tenant can run concurrently. The limit of maximum number of jobs per tenant is controlled by the query-frontend value  `max_outstanding_per_tenant`.
