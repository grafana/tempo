...
Update the values in this Helm chart configuration to match your S3 bucket and credentials.
...
Set the `endpoint` to `insecure` when using `http://`, and configure addressing style if required by the provider.

Refer to the [Tempo Helm chart values](https://grafana.com/docs/helm-charts/tempo-distributed/next/get-started-helm-charts/#tempo-helm-chart-values) documentation for a more complete example.
...
## Permissions IAM policy example
