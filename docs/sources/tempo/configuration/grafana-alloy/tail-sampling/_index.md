...
## Pipeline workflows
 ...
 When implementing tail sampling into your telemetry collection pipeline, there are some considerations that should be applied. Primarily the act of sampling reduces the amount of tracing telemetry data that is sent to Grafana Tempo, or Grafana Cloud Traces. This can have an effect on observation of data inside Grafana and Grafana Cloud.

For example, Grafana Application Observability uses the metrics generated by Grafana Cloud Traces to power the underlying RED and service graph metrics that allow users to easily determine their application service status. Should sampling be occurring before traces are sent to Grafana Cloud, the reduced trace volume translates to a potentially inaccurate view of these generated metrics as the count for these metrics cannot include the spans that have been dropped. Fortunately, Grafana Application Observability can be configured to accept metrics generated from elsewhere, to still ensure a full and robust view of your application and infrastructure estates. &lt;WE SHOULD ADD LINKS TO THE CONFIG OPTIONS>

The following is a suggested pipeline that can be applied to both Grafana Alloy and the OpenTelemetry Collector, to carry out tail sampling, but also ensure that other telemetry signals are still fully rounded for observation from within Grafana and Grafana Cloud.
...
* The **Prometheus Exporter** is optional**.** If metrics are not sent via OTLP, then this component can be used to send Prometheus compatible metrics to [Mimir](https://grafana.com/oss/mimir/) or [Grafana Cloud Metrics](https://grafana.com/products/cloud/metrics/).