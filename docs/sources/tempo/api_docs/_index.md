...
- `span_pruning_max_parent_depth = (integer)`
  ...
  Default = `1`
- `q = (TraceQL filter)`
  Optional. A single TraceQL spanset filter (for example `{ .http.status_code = 500 }`) that returns only the matching spans. Set `keep_hierarchy=true` to also include each match's ancestor path. It supports span, resource, event, link, and instrumentation-scope attributes and intrinsics across string, integer, float, boolean, and array values, with all comparison and regex operators and existence checks (`{ span.foo != nil }` and `{ span.foo = nil }`). `event:` and `link:` conditions match against all of a span's events and links. Only a single `{ ... }` filter is supported. Pipelines, structural operators, metrics queries, and trace-level intrinsics (`trace:duration`, `trace:rootName`, `trace:rootService`, `trace:id`) return a `400`. An absent or empty `q`, `{}`, and `{ true }` return the full trace. When the filter drops spans - including when nothing matches, which returns `200` with an empty trace - the response status is `PARTIAL` to signal it is a subset, not the complete trace. Results match a normal search except for a few edge cases, such as negating an array attribute against one of its own members (`{ span.tags != "a" }` when `tags` contains `a`) and unscoped not-exists (`{ .foo = nil }`).
- `keep_hierarchy = (bool)`
