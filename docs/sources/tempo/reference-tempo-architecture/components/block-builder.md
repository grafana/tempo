...
The `nocompact.flg` file is written before `meta.json` to prevent the compactor from touching the block while it's still being built. After the block-builder finishes its cycle, it removes this flag. This prevents a race condition where a compactor might try to compact a block that's about to be replaced.
...
Block-builders are stateless beyond their scratch disk. Scaling up is straightforward — add instances and let the consumer group rebalance. Scaling down causes remaining instances to pick up orphaned partitions.
...
Refer to the [block-builder configuration](https://grafana.com/docs/tempo/<TEMPO_VERSION>/configuration/#block-builder) for the full list of options.
