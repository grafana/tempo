...
They consume from Kafka independently of block-builders and hold traces in memory, making them available for queries as soon as they're consumed.

Live-stores exist because there's a gap between when you write data to Kafka and when the block-builder flushes it to object storage.
...
| Compactor | Maintenance | Compacts and deduplicates blocks, enforces retention |
