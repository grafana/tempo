...
Tempo 2.3 makes [vParquet3]({{< relref "../configuration/parquet" >}}), a Parquet version designed to be more compatible with other Parquet implementations, available as a production-ready option.
This block format is required for using structural operators and improves query performance relative to previous formats.
...
* Added quoted attribute syntax so you can search for attributes with terminal characters like periods (`.`) in their names ([documentation]({{< relref "../traceql#experimental-structural" >}})) [[PR 3004](https://github.com/grafana/tempo/pull/3004)]
