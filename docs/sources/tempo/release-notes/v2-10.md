...
These constants make queries more readable and help avoid errors when working with extreme integer values:

```traceql
{ span.counter >= minInt && span.counter <= maxInt }
```

Using `minInt` and `maxInt` is clearer than manually typing the full numeric values.
