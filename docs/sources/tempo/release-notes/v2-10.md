...
Use named constants to represent the minimum and maximum 64-bit signed integer values (`-9223372036854775808` and `9223372036854775807`). [[PR 5982](https://github.com/grafana/tempo/pull/5982)]

These constants are useful for detecting data quality issues where numeric attributes contain unexpected extreme values:

```traceql
{ span.queue_depth = maxInt }
```

This query finds spans where `queue_depth` is set to the maximum integer value, which some systems use as a sentinel to indicate an overflow or "infinite" queue. Using `maxInt` makes the query readable and avoids errors from manually typing out the 19-digit number.

### Metrics-generator

Tempo 2.10 introduces improved cardinality management for the metrics-generator. Entity-based limiting is a new alternative to series-based limiting. It preserves complete metrics data for each logical entity rather than randomly dropping series when limits are reached. [[PR 5788](https://github.com/grafana/tempo/pull/5788)]
...
vParquet5 improves query speed and reduces memory usage with these changes. Dedicated columns now support array values, enabling efficient storage for multi-valued attributes like HTTP headers. [[PR 5760](https://github.com/grafana/tempo/pull/5760), [documentation](/docs/tempo/<TEMPO_VERSION>/operations/dedicated_columns/#array-valued-attributes)] Virtual span row numbers speed up queries that filter only on resource attributes. [[PR 5943](https://github.com/grafana/tempo/pull/5943)]

Dedicated columns now support event-level attributes. A new "blob" detection system identifies high-cardinality attributes like UUIDs and stack traces, then applies optimized encoding to reduce memory pressure and improve performance. The format automatically removes unused dedicated columns to save storage. [[PR 5946](https://github.com/grafana/tempo/pull/5946), [documentation](/docs/tempo/<TEMPO_VERSION>/operations/dedicated_columns/#blob-attributes)]
