...
Quickly identify gaps in your instrumentation by finding spans where expected attributes are absent. [[PR 4905](https://github.com/grafana/tempo/pull/4905), [documentation](/docs/tempo/<TEMPO_VERSION>/traceql/construct-traceql-queries/#comparison-operators)].
...
This query finds all traces where the service.version attribute is missing at the resource level—useful for ensuring all services are properly versioned before a release.

#### Query span fan-out with `span:childCount`

Analyze trace structure by finding spans based on how many child operations they spawn.
...
- Identify terminal operations to identify database calls or external API requests
- Detect high fan-out patterns to identify N+1 query problems or excessive parallelism

```traceql
{ span:childCount > 10 }
```

This query finds spans with high fan-out—spawning more than 10 child operations—which often reveals inefficient patterns like parallel loops or batch explosions.

#### Cleaner numeric bounds with `minInt` and `maxInt`

Use readable constants instead of magic numbers for integer comparisons. [[PR 5982](https://github.com/grafana/tempo/pull/5982)]
These constants make queries self-documenting and less error-prone:

```traceql
{ span.http.status_code > minInt && span.http.status_code < maxInt }
```
...
**Attribute = nil support**: TraceQL now supports checking if attributes are nil using `attribute = nil` syntax, enabling queries for traces with missing attributes.

**span:childCount intrinsic**: New TraceQL intrinsic `span:childCount` which is the number of direct children of the span. Supported in vParquet5 and later.

**minInt/maxInt constants**: Added `minInt` and `maxInt` constants to TraceQL for representing minimum and maximum integer values without hardcoding long numeric literals. Example: `{ span.http.status_code > minInt }`.
