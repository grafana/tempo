...
To learn more about this release, refer to the [Tempo 3.0 blog post](https://grafana.com/blog/tempo-3-0-release-all-the-latest-features/).
...
### Span name sanitization (experimental)
...
To use the capability, set the `span_name_sanitization` option in the `overrides` block of your Tempo configuration:
...
Refer to [Reduce cardinality with span name sanitization](/docs/tempo/<TEMPO_VERSION>/metrics-from-traces/metrics-generator/reduce-cardinality/) for details. [[PR 6098](https://github.com/grafana/tempo/pull/6098)]

### Span metrics filter improvements
...
### Service graph filter improvements
...
- Fail-on-high-lag enabled by default: `live_store.fail_on_high_lag` now defaults to `true` (was `false`). When a live-store's Kafka lag overlaps a query's time range, search and metrics requests return an error rather than silently incomplete results, trading availability for correctness. To restore the previous behavior, set `live_store.fail_on_high_lag: false`. Accordingly, `query_frontend.query_end_cutoff` now defaults to `30s` (was `0`), excluding the most recent 30 seconds from queries to avoid incomplete results; it must be less than `query_frontend.search.query_backend_after`. [[PR 7210](https://github.com/grafana/tempo/pull/7210)]
