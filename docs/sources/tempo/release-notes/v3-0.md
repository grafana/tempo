...
Span name clustering uses the DRAIN algorithm to automatically group high-cardinality span names and replace variable segments with a `<_>` placeholder. For example, `GET /users/123` and `GET /users/456` both map to `GET /users/<_>`, reducing active series without instrumentation changes. A `dry_run` mode lets you evaluate the cardinality reduction before applying it. [[PR 6098](https://github.com/grafana/tempo/pull/6098), [documentation](/docs/tempo/<TEMPO_VERSION>/metrics-from-traces/metrics-generator/reduce-cardinality/)]
