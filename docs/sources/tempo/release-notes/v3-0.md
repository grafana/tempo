...
- [TraceQL metrics generally available](#traceql-metrics): Production-ready trace-derived metrics with new comparison operators (alerting is experimental)
...
- [Query performance improvements](#query-performance-improvements): vParquet5 production-ready with doubled dedicated columns, plus TraceQL AST optimization
...
Traces flow directly from the distributor to the live store. Previously known as "Project Rhythm" (experimental in 2.9 and 2.10), this architecture is now the default and only ingest path.
...
Ingesters and the old compactor are replaced by block-builders, live-stores, and the backend scheduler/worker.
Job-based compaction through the scheduler and worker tracks progress centrally, automatically reschedules failed jobs, and eliminates duplicated compaction work.
