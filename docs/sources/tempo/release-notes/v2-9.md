...
This allows you to more easily understand interactions between your services, as well as investigate and diagnose issues faster.
...
Project Rhythm is the codename for Tempo's new architecture project.
...
- Eliminate the requirement for RF3 -> TraceQL metrics GA
- Decouple the read and write path -> Scalability and reliability
- Lay out the foundation to significantly reduce TCO -> Room for growth
...
- **Low write reliability**: in order to reduce the impact that the generators have on the read path, generator ingestion is asynchronous and doesn't return errors to the client. While this is important to protect the ingester's write path, it adds unreliability to generator's ingestion.
...
- Fixed Tempo configuration options that are always overrided with configuration overrides section. ([PR 5202](https://github.com/grafana/tempo/pull/5202))
