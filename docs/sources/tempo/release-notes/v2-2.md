...
With this release, we’ve added to the [TraceQL language](https://grafana.com/docs/tempo/latest/traceql/). 
TraceQL now offers: 
...
To learn more about the TraceQL syntax, see the [TraceQL documentation](http://localhost:3002/docs/tempo/latest/traceql/). 
For information on planned future extensions to the TraceQL language, see [future work](http://localhost:3002/docs/tempo/latest/traceql/architecture/).
...
To learn more about this API, refer to the[ metrics summary API documentation.](https://grafana.com/docs/grafana-cloud/data-configuration/traces/metrics-summary-api/)
...
* Tempo’s [tag values](https://grafana.com/docs/tempo/latest/api_docs/#search-tag-values) and [tag names](https://grafana.com/docs/tempo/latest/api_docs/#search-tags) APIs now support filtering [PR [2253](https://github.com/grafana/tempo/pull/2253)]. This lets you retrieve all valid attribute values and names given certain criteria. For example, you can get a list of values for the attribute `namespace` seen on spans with attribute `resource=A.` This feature is off by default; to enable, configure `autocomplete_filtering_enabled`. ([documentation]({{< relref "../api_docs" >}})). Grafana’s autocomplete can make use of this filtering capability to provide better suggestions starting in v10.1 [[PR 67845]](https://github.com/grafana/grafana/pull/67845).
...
* Tempo’s metrics-generator can now detect virtual nodes ([documentation]({{< relrelf "../metrics-generator/service_graphs#virtual-nodes" >}})) [PR [2365](https://github.com/grafana/tempo/pull/2365)]. As a result, you’ll now see these virtual nodes represented in your service graph. For more information, refer to the [virtual nodes documentation]({{<> relrelf "../metrics-generator/service_graphs#virtual-nodes" >}}. 


## Upgrade considerations

When [upgrading](http://localhost:3002/docs/tempo/latest/setup/upgrade/) to Tempo 2.2, be aware of these breaking changes:
...
While not a breaking change, upgrading to Tempo 2.2 will by default change Tempo’s block format to vParquet2. To stay on a previous block format, read the[ Parquet configuration documentation](https://grafana.com/docs/tempo/next/configuration/parquet/#choose-a-different-block-format). Note that we strongly encourage upgrading to vParquet2 as soon as possible as this is required for using structural operators in your TraceQL queries and provides query performance improvements, in particular on queries using the `duration` intrinsic. 
...
* Fixed an issue where Tempo failed to find the `meta.json` path after adding a prefix in object storage configuration. [PR [2585](https://github.com/grafana/tempo/issues/2585)] 
