...
- The MCP server can now be enabled with the command-line flag `--query-frontend.mcp-server.enabled=true` instead of requiring a YAML configuration change. This makes it easier to enable the [MCP server](/docs/tempo/<TEMPO_VERSION>/api_docs/mcp-server/) in environments like the `grafana/otel-lgtm` Docker image where appending YAML configuration is inconvenient. [[PR 6903](https://github.com/grafana/tempo/pull/6903)]
...
When [upgrading](/docs/tempo/<TEMPO_VERSION>/set-up-for-tracing/setup-tempo/upgrade-2.x/) to Tempo 2.10, be aware of these considerations and breaking changes.
...
- S3 SSE-C `encryption_key` is now treated as a secret to prevent exposure in logs, config dumps, and API responses. The field type changed from a plain string to a redacted secret, matching the existing handling of `secret_key` and `session_token`. This affects deployments using S3 SSE-C encryption, which was introduced in Tempo 2.10.0. Addresses [CVE-2026-28377](https://github.com/advisories/CVE-2026-28377). [[PR 6711](https://github.com/grafana/tempo/pull/6711)]
