...
**2026-07-16 amendment — the queue bound doesn't reach into the Kafka client.** The bound above governs what the consumer has already handed to the worker pool; it does not bound what the underlying Kafka client buffers internally ahead of that hand-off. Because every instance consumes all `K` partitions (§ Consumers) rather than a partition-ring-sharded subset the way block-builder and live-store do, the shared `pkg/ingest` client defaults (50 MiB/partition, 100 MiB/broker response — sized for their one-partition-class-per-instance case) leave room for a many-partition replay burst to buffer far more inside the client itself than the queue's own admission control ever sees. This consumer now sets tighter, explicit caps instead of inheriting those shared defaults: 8 MiB/partition, 64 MiB/broker response, 128 MiB/broker read. See the restore-memory amendment (§ Snapshots) for the incident this was one of two contributing factors in. This 8 MiB partition cap is also why producer-side `chunk_size` (`pkg/bloomgatewayevents.Config`) is bounded at 250,000 trace IDs (~4 MiB, half the cap): the invariant chain is chunk_size ≤ 4 MiB ⇒ every individual `AddChunk` message stays comfortably under the 8 MiB partition fetch cap, with headroom left over for several chunks to batch through one fetch rather than one message alone approaching that ceiling.
...
- **Save memory (2026-07-16 amendment).** Save streams complete leaves one at a time — collect owned indexes cheaply (a `Directory.Range` pass over indexes only, no cloning), then clone-and-serialize each leaf individually, discarding it immediately — rather than cloning every owned leaf into memory before writing anything. Peak *additional* memory during a save is therefore O(one leaf), never O(owned leaves). The earlier bulk-clone-then-serialize shape doubled live heap at production scale and OOM-killed the pod mid-assembly on every snapshot tick, with no snapshot file ever produced (incident: ~2.1M owned leaves / ~7.3 GiB of cloned leaf data landing on an already ~11.9 GiB heap against a 13.74 GiB GOMEMLIMIT / 16 GiB cgroup limit). A leaf that stops being complete between collection and cloning (an ownership change shedding it, most plausibly) is skipped outright — safe by the completeness invariant (§ Leaf lifecycle): a leaf missing from the snapshot is simply re-enqueued for reconstruction on the next load. The on-disk format is unchanged; the leaf count is written as a placeholder and patched once streaming finishes (skipped leaves shrink the true count below what was initially collected), and the trailing checksum is computed from the finalized file rather than incrementally — one extra bounded-memory sequential read over the body, comparable in size to the write itself, so it roughly doubles the pause budgeted above. **Measured, not estimated:** a full 7.43 GiB save cycle paused the apply path for **117s on gp3 EBS** (write + CRC re-read, ~150 MB/s combined effective throughput) — the ~10–20s figure in § Consistency above is the expectation for production hardware (local NVMe); gp3 EBS is the honest dev-cluster number for anyone running on network-attached volumes instead, and the gap between the two is exactly the extra re-read pass, not a discrepancy.
- **Restore memory (2026-07-16 amendment).** `Load` now decodes leaves one at a time and hands each straight to a sink — the Directory's own ownership-checked `Complete` call, in the production path — instead of first reading the entire on-disk body into memory and building a complete `CompleteLeaves` map for `reconcileStartup` to import afterward. Leaves have always been handed to the Directory **by pointer**, never copied, at the import step; the old shape's extra cost was not duplicated leaf data but two forms of pure overhead sitting on top of it: the map itself, and — since the old reader buffered the *whole* body into one contiguous slice before decoding any of it — the full on-disk size (7.44 GiB in the incident below), resident start-to-end regardless of how much of it had already been logically consumed. That overhead does not vanish the instant `Load` returns; it becomes GC-eligible garbage that lingers until collected, colliding with the Kafka replay burst across all `K` partitions (§ Consumers) and reconciliation startup work that both begin immediately once the gateway reports ready. That collision — not decode itself — is why the incident below OOMed 17s *after* `Tempo started` logged. Streaming removes both the map and the whole-body buffer: registry blocks/tenants/tokens/constructing-ranges (small) still get materialized, but leaf payloads flow straight from file to Directory with no intermediate holding structure, so restore peak memory is O(one leaf) plus that small registry state — symmetric to the save-side amendment above.
  - **The checksum hazard streaming introduces.** The old code validated the trailing checksum as part of a single read-and-decode pass and only returned state — and so only let `reconcileStartup` import anything — after that validation succeeded, meaning a corrupt file never reached the Directory. Decoding and importing in one streaming pass would break that guarantee: a corrupt tail would only be discovered after earlier leaves in the file had already been imported into the live, query-serving Directory, leaving it partially populated from a file that turned out to be invalid — a new failure mode with no equivalent before. `Load` avoids it by reading the body **twice**: a first bounded-memory pass computes the checksum over the whole body and compares it against the trailer before anything is decoded or imported, and only on success does a second pass decode and stream leaves into the sink. Both passes are O(1) memory; the cost is one extra full sequential read, the same shape of trade the save-side amendment above already accepts for its own re-verification pass.
  - **Incident:** pod-10 (16 GiB limit, 13.74 GiB `GOMEMLIMIT`) restored a 7.44 GiB snapshot (`complete_leaves=2098208`) in ~3 min, resumed all 16 Kafka partitions, logged `Tempo started` — then was OOMKilled 17s later, twice in a row. Restored steady state alone is ~13.8 GiB heap (1.22×10⁹ entries): the margin to the 16 GiB limit could not also absorb decode-phase transients on top of the 16-partition replay burst and reconciliation startup. The cell was bumped to 20 GiB as a stopgap pending this fix. This streaming rewrite addresses the decode-transient contributor; the fetch caps in § Write path "Backpressure and memory pressure" address the replay-burst contributor — together they bring restore peak back to roughly steady state, so 16 GiB is sufficient again.
