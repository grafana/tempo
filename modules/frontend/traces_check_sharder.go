...
type asyncTracesCheckSharder struct {
	next            pipeline.AsyncRoundTripper[combiner.PipelineResponse]
	cfg             *TracesCheckConfig
	logger          log.Logger
	blockBoundaries [][]byte
}

func newAsyncTracesCheckSharder(cfg *TracesCheckConfig, logger log.Logger) pipeline.AsyncMiddleware[combiner.PipelineResponse] {
	return pipeline.AsyncMiddlewareFunc[combiner.PipelineResponse](func(next pipeline.AsyncRoundTripper[combiner.PipelineResponse]) pipeline.AsyncRoundTripper[combiner.PipelineResponse] {
		return asyncTracesCheckSharder{
			next:            next,
			cfg:             cfg,
			logger:          logger,
...
func (s asyncTracesCheckSharder) RoundTrip(pipelineRequest pipeline.Request) (pipeline.Responses[combiner.PipelineResponse], error) {
	ctx, span := tracer.Start(pipelineRequest.Context(), "frontend.ShardTracesCheckQuery")
	defer span.End()
...
// block boundaries
func (s *asyncTracesCheckSharder) buildShardedRequests(parent pipeline.Request) ([]pipeline.Request, error) {
	...
	var bodyBytes []byte
	if parent.HTTPRequest().Body != nil {
		bodyBytes, err = io.ReadAll(parent.HTTPRequest().Body)
		if err != nil {
			return nil, err
		}
		parent.HTTPRequest().Body.Close()
	}
