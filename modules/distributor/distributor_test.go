...
func TestPushTracesToLocalLiveStore(t *testing.T) {
	...
	distributorCfg, clientCfg, overridesSvc, _, ingesterRing, loggingLevel, middleware := setupDependencies(t, limits)
...
func prepare(t *testing.T, limits overrides.Config, logger kitlog.Logger) (*Distributor, map[string]*mockIngester) {
	...
	d, err := New(distributorConfig, clientConfig, ingestersRing, LocalPushTargets{}, nil, overrides, mw, logger, l, prometheus.NewPedanticRegistry())
	require.NoError(t, err)
