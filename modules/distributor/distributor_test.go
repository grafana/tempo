...
func TestPushTracesSkipMetricsGenerationIngestStorage(t *testing.T) {
	...
	t.Run("with no-generate-metrics header", func(t *testing.T) {
		...
		// context.
		ctx := metadata.NewIncomingContext(ctx, metadata.Pairs(NoGenerateMetricsContextKey, ""))
		_, err = d.PushTraces(ctx, traces)
		require.NoError(t, err)
		...
		fetches.EachRecord(func(record *kgo.Record) {
			...
			require.Contains(t, record.Headers, kgo.RecordHeader{Key: generator.NoGenerateMetricsContextKey})
		})
