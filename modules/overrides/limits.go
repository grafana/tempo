...
func (l *Limits) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&l.IngestionRateLimitBytes, "distributor.ingestion-rate-limit-bytes", 10e6, "Per-user ingestion rate limit in bytes per second.")
	f.IntVar(&l.IngestionBurstSizeBytes, "distributor.ingestion-burst-size-bytes", 15e6, "Per-user ingestion burst size in bytes. Should be set to the expected size (in bytes) of a single push request.")
