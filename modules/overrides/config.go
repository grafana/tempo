...
func (c *Config) RegisterFlagsAndApplyDefaults(f *flag.FlagSet) {
	...
	c.Defaults.Ingestion.RateLimitBytes = f.Int("distributor.ingestion-rate-limit-bytes", 30_000_000, "Per-user ingestion rate limit in bytes per second.")
	c.Defaults.Ingestion.BurstSizeBytes = f.Int("distributor.ingestion-burst-size-bytes", 30_000_000, "Per-user ingestion burst size in bytes. Should be set to the expected size (in bytes) of a single push request.")
