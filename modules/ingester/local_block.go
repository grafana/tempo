...
func (c *LocalBlock) TracesCheck(ctx context.Context, ids []common.ID, opts common.SearchOptions) (map[string]bool, uint64, error) {
	ctx, span := tracer.Start(ctx, "LocalBlock.TracesCheck")
	defer span.End()
	return c.BackendBlock.TracesCheck(ctx, ids, opts)
}
