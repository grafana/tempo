...
func TestInstanceSearchTagAndValuesV2IDs(t *testing.T) {
	...
	_, _, _, _ = writeTracesForSearch(t, i, spanName, foo, bar, true, false)

	userCtx := user.InjectOrgID(context.Background(), "fake")

	t.Run("Span ID", func(t *testing.T) {
		tagValuesResp, err := i.SearchTagValuesV2(userCtx, &tempopb.SearchTagValuesRequest{
			TagName: "span:id",
			Query:   queryThatMatches,
		})
		require.NoError(t, err)
	...
	t.Run("Link Trace ID", func(t *testing.T) {
		tagValuesResp, err := i.SearchTagValuesV2(userCtx, &tempopb.SearchTagValuesRequest{
			TagName: "link:traceID",
			Query:   queryThatMatches,
		})
		require.NoError(t, err)
	...
	t.Run("Link Span ID", func(t *testing.T) {
		tagValuesResp, err := i.SearchTagValuesV2(userCtx, &tempopb.SearchTagValuesRequest{
			TagName: "link:spanID",
			Query:   queryThatMatches,
		})
