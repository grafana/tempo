...
func TestInstanceSearchMetrics(t *testing.T) {
	...
	require.Equal(t, uint32(1), m.TotalBlocks)     // 1 head block
	...
	require.Equal(t, numTraces, m.InspectedTraces)
	require.Less(t, numBytes, m.InspectedBytes)
	require.Equal(t, uint32(2), m.TotalBlocks) // 1 head block, 1 completing block
	...
	m = search()
	require.Equal(t, numTraces, m.InspectedTraces)
	require.Equal(t, uint32(2), m.TotalBlocks) // 1 head block, 1 complete block
}
