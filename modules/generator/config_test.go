...
func TestProcessorConfig_copyWithOverrides(t *testing.T) {
	original := &ProcessorConfig{
		...
		SpanMetrics: spanmetrics.Config{
			...
			IntrinsicDimensions: spanmetrics.IntrinsicDimensions{Job: true},
		},
	}
	...
	t.Run("overrides buckets and dimension", func(t *testing.T) {
		...
		assert.Equal(t, spanmetrics.IntrinsicDimensions{Job: true}, original.SpanMetrics.IntrinsicDimensions)
		...
		assert.Equal(t, spanmetrics.IntrinsicDimensions{Job: true, StatusCode: true}, copied.SpanMetrics.IntrinsicDimensions)
	})
