...
type nativeHistogramAccumulator struct {
	...
	positiveBucketKey    int
	positiveBucketCount  int64
	positiveBucketSet    bool
	...
	exemplarBuf [10]nativeHistogramAccumulatorExemplar
...
func useNativeHistogramAccumulator(hasClassic bool, bucketFactor float64) bool {
...
func (a *nativeHistogramAccumulator) observe(value float64, traceID string, multiplier float64, _ string, timestamp time.Time) {
	ex := nativeHistogramAccumulatorExemplar{
		labelValue: traceID,
	}
	a.observeWithExemplar(value, ex, multiplier, timestamp)
}

func (a *nativeHistogramAccumulator) observeTraceIDBytes(value float64, traceID []byte, multiplier float64, _ string, timestamp time.Time) {
	ex := nativeHistogramAccumulatorExemplar{}
...
func (a *nativeHistogramAccumulator) observeWithExemplar(value float64, ex nativeHistogramAccumulatorExemplar, multiplier float64, timestamp time.Time) {
	if multiplier == 1 {
		a.observeOne(value, ex, timestamp)
		return
	}
...
func (a *nativeHistogramAccumulator) observeOne(value float64, ex nativeHistogramAccumulatorExemplar, timestamp time.Time) {
...
func (a *nativeHistogramAccumulator) clearExemplars() {
	clear(a.exemplars)
	a.exemplars = a.exemplarBuf[:0]
}

func (a *nativeHistogramAccumulator) addExemplar(value float64, ex nativeHistogramAccumulatorExemplar, timestamp time.Time) {
	ex.value = value
...
func (a *nativeHistogramAccumulator) reset() {
	...
	a.sum = 0
	a.zeroCount = 0
	a.positiveBucketKey = 0
	a.positiveBucketCount = 0
	a.positiveBucketSet = false
	clear(a.positiveBucketCounts)
	clear(a.negativeBucketCounts)
}

func (a *nativeHistogramAccumulator) populatedBuckets() int {
	total := len(a.positiveBucketCounts) + len(a.negativeBucketCounts)
	if a.positiveBucketSet {
		total++
	}
	return total
}
...
func (a *nativeHistogramAccumulator) doubleBucketWidth() {
	...
	a.schema--
	a.mergePositiveBucketsForLowerSchema()
	a.mergeBucketsForLowerSchema(a.negativeBucketCounts)
}

func (a *nativeHistogramAccumulator) addPositiveBucket(key int, count int64) {
	if a.positiveBucketCounts != nil {
		a.positiveBucketCounts[key] += count
		return
	}
	if !a.positiveBucketSet {
		a.positiveBucketKey = key
		a.positiveBucketCount = count
		a.positiveBucketSet = true
		return
	}
	if a.positiveBucketKey == key {
		a.positiveBucketCount += count
		return
	}

	a.positiveBucketCounts = make(map[int]int64, 2)
	a.positiveBucketCounts[a.positiveBucketKey] = a.positiveBucketCount
	a.positiveBucketCounts[key] = count
	a.positiveBucketKey = 0
	a.positiveBucketCount = 0
	a.positiveBucketSet = false
...
func (a *nativeHistogramAccumulator) mergePositiveBucketsForLowerSchema() {
	if a.positiveBucketCounts != nil {
	...
	a.positiveBucketKey = bucketKeyForLowerSchema(a.positiveBucketKey)
}
...
func (a *nativeHistogramAccumulator) mergeBucketsForLowerSchema(buckets map[int]int64) {
	...
	for key, count := range buckets {
		mergedKey := bucketKeyForLowerSchema(key)
...
func bucketKeyForLowerSchema(key int) int {
	if key > 0 {
		key++
	}
	return key / 2
...
func nativeAccumulatorBuckets(
	...
	if bucketCounts == nil {
		...
		spans = append(spans[:0], promhistogram.Span{
			Offset: int32(singleKey),
			Length: 1,
		})
		deltas = append(deltas[:0], singleCount)
	...
	for schema := range boundsBySchema {
		...
		for bucket := 0; bucket < numBuckets-1; bucket++ {
			...
			if schema > 0 && (bucket+1)%2 == 0 {
				previousSchema := schema - 1
				if previousSchema >= 0 && previousSchema < len(boundsBySchema) {
					previousBounds := boundsBySchema[previousSchema]
					previousIndex := bucket/2 + 1
					if previousIndex < len(previousBounds) {
						bound = previousBounds[previousIndex]
					}
				}
			}
			if bound == 0 {
