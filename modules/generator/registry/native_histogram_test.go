...
func Test_nativeHistogramOverrideHashMatchesBinaryWriteFNV(t *testing.T) {
	bucketFactor := 1.1
	maxBucketNum := uint32(16)
	minResetDur := 15 * time.Minute

	want := fnv.New64a()
	require.NoError(t, binary.Write(want, binary.LittleEndian, bucketFactor))
	require.NoError(t, binary.Write(want, binary.LittleEndian, maxBucketNum))
	require.NoError(t, binary.Write(want, binary.LittleEndian, minResetDur))

	got := fnv64aAddUint64(fnv64aOffset, math.Float64bits(bucketFactor))
	got = fnv64aAddUint32(got, maxBucketNum)
	got = fnv64aAddUint64(got, uint64(minResetDur))
...
// Test_NativeOnly_clearsExemplarsAfterCollect verifies that exemplars are not
// re-emitted on a subsequent collect when no new observations occurred between
// scrapes. Regression test for the native-accumulator exemplar-leak finding.
func Test_NativeOnly_clearsExemplarsAfterCollect(t *testing.T) {
	overrides := &mockOverrides{
		nativeHistogramBucketFactor:     1.5,
		nativeHistogramMaxBucketNumber:  10,
		nativeHistogramMinResetDuration: time.Minute,
	}
	h := newNativeHistogram("test_native_clear", []float64{1, 2}, noopLimiter, "trace_id", HistogramModeNative, nil, testTenant, overrides, 15*time.Minute)
	lbls := buildTestLabels([]string{"service"}, []string{"svc"})
	h.ObserveWithExemplar(lbls, 0.5, "trace-1", 1.0)
	h.ObserveWithExemplar(lbls, 1.5, "trace-2", 1.0)

	ts := time.Now().UnixMilli()
	first := &capturingAppender{}
	require.NoError(t, h.collectMetrics(first, ts))
	require.Len(t, first.exemplars, 2)

	// Second collect, no new observations → no exemplars.
	second := &capturingAppender{}
	require.NoError(t, h.collectMetrics(second, ts+1))
	require.Empty(t, second.exemplars, "exemplars must not be re-emitted after the previous collect cleared them")
}
