...
func Test_gaugeInc(t *testing.T) {
	...
	c := newGauge("my_gauge", onAdd, nil, nil, time.Duration(15*time.Minute))
...
func Test_gauge_removeStaleSeries(t *testing.T) {
	...
	c := newGauge("my_gauge", nil, onRemove, nil, time.Duration(15*time.Minute))
...
func Test_gauge_externalLabels(t *testing.T) {
	c := newGauge("my_gauge", nil, nil, map[string]string{"external_label": "external_value"}, time.Duration(15*time.Minute))
...
func Test_gauge_concurrencyDataRace(t *testing.T) {
	c := newGauge("my_gauge", nil, nil, nil, time.Duration(15*time.Minute))
