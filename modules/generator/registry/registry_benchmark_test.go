...
// costs are visible. The label set is identical every iteration, so after the
// first iteration the sanitizer takes its exact-match path and the limiter has
// already cloned the label names it retains.
