...
type Limiter interface {
	// Transform is called for set of labels before it is used in a series. This
	// gives the limiter a chance to transform labels to reduce cardinality. For
	// example, a limiter could combine measurements across multiple series by
	// aggregating them into a single series. It returns the transformed labels
	// or the original if no transformation was applied.
	Transform(lbls labels.Labels) labels.Labels
...
func (r *ManagedRegistry) NewLabelBuilder() LabelBuilder {
	return NewLabelBuilder(r.cfg.MaxLabelNameLength, r.cfg.MaxLabelValueLength, r.Transform)
}

func (r *ManagedRegistry) Transform(lbls labels.Labels) labels.Labels {
	r.entityDemand.Insert(lbls.Hash())
	return r.limiter.Transform(lbls)
}
