...
func TestTargetInfoWithDifferentBatches(t *testing.T) {
	...
	assert.Equal(t, 0.0, testRegistry.Query("target_info", lbls))
	assert.Equal(t, 1.0, testRegistry.Query("target_info", lbls2))
	assert.Equal(t, 1.0, testRegistry.Query("target_info", lbls3))
}
