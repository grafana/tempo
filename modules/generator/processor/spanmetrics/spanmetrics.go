...
func New(cfg Config, reg registry.Registry, filteredSpansCounter, invalidUTF8Counter prometheus.Counter) (gen.Processor, error) {
	...
	// Sanitized dimension label names are deterministic; compute them once at
	// construction instead of once per span.
...
// spanKindString and statusCodeString return exactly the proto enum .String()
// values for known enum members without the map lookup, falling back to
