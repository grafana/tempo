...
func (p *Processor) aggregateMetrics(resourceSpans []*v1_trace.ResourceSpans) {
	...
	for _, rs := range resourceSpans {
		...
		resourceFilter := p.filter.ForResource(rs.Resource)
		// target_info build + emit is deferred to the first span that passes
		// its OWN UTF-8 validation (see aggregateMetricsForSpan). This matches
		// main's behavior: a resource whose every passing span has invalid
		// UTF-8 must NOT emit target_info. The state struct caches the
		// resource-level computation across spans so we don't redo it.
		ti := targetInfoBatchState{
			resourceLabels: resourceLabels,
			resourceValues: resourceValues,
		}
		for _, ils := range rs.ScopeSpans {
			for _, span := range ils.Spans {
				if resourceFilter.Apply(span) {
					var traceID []byte
					if p.Cfg.Subprocessors[Latency] {
						traceID = span.TraceId
					}
					p.aggregateMetricsForSpan(svcName, jobName, instanceID, rs.Resource, span, traceID, &ti, resourceDimensionValues, resourceDimensionFound, updateTimeMs)
					continue
				}
				p.filteredSpansCounter.Inc()
			}
		}
		// Carry slice growth (if getTargetInfoAttributesValues reallocated)
		// back to the outer scope so subsequent resources can reuse it.
		resourceLabels = ti.resourceLabels
		resourceValues = ti.resourceValues
	}
}

// targetInfoBatchState carries the lazy build state for the target_info
// metric across spans of a single resource. The build is deferred until the
// first span passes its own UTF-8 check, so a resource with no valid span
// labels never emits target_info.
type targetInfoBatchState struct {
	// resourceLabels and resourceValues are mutated by getTargetInfoAttributesValues.
	// They are passed in pre-allocated so the buffer is reused across resources.
	resourceLabels []string
	resourceValues []string
	// handled is true once buildAndSetTargetInfoLabels has been called for this
	// resource. valid is its result (UTF-8 check on the resource label set).
	handled bool
	valid   bool
}

func (p *Processor) aggregateMetricsForSpan(svcName string, jobName string, instanceID string, rs *v1.Resource, span *v1_trace.Span, traceID []byte, ti *targetInfoBatchState, resourceDimensionValues []string, resourceDimensionFound []bool, updateTimeMs int64) {
	...
	if p.Cfg.EnableTargetInfo {
		if !ti.handled {
			// First span for this resource that passed UTF-8 validation:
			// build and emit target_info now. Subsequent spans reuse the
			// cached `valid` result without rebuilding.
			getTargetInfoAttributesValues(&ti.resourceLabels, &ti.resourceValues, rs.Attributes, p.targetInfoExcluded, p.sanitizeCache.Get)
			ti.valid = p.buildAndSetTargetInfoLabels(ti.resourceLabels, ti.resourceValues, jobName, instanceID, updateTimeMs)
			ti.handled = true
		}
		if !ti.valid {
...
func getResourceServiceLabels(attributes []*v1_common.KeyValue, jobNameBuf []byte) (svcName string, jobName string, instanceID string, returnedJobNameBuf []byte) {
	...
	return svcName, string(jobNameBuf), instanceID, jobNameBuf
}
