...
// dimensionLabel carries the precomputed sanitized label names for a
// configured dimension, so the per-edge paths avoid repeated cache lookups
...
// buildKeyFromBytes hex-encodes both IDs directly into the key, avoiding the
// two intermediate hex-string allocations of buildKey(hex(...), hex(...)).
