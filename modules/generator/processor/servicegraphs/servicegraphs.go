...
func updateServerEdge(e *store.Edge, u serverEdgeUpdate) {
	...
	e.SpanMultiplier = u.spanMultiplier
	if u.root {
		// PeerNode is only read in onExpire (virtual-node inference) and only
		// for root server spans whose CLIENT counterpart is missing: line ~623
		// requires `parentSpan == ""`. Non-root SERVER spans either pair with
		// a CLIENT (which sets PeerNode itself if applicable) or expire
		// without that path triggering — so setting PeerNode here would never
		// be observed. Gating saves a per-span FindAttributeValue scan.
		u.p.upsertPeerNode(e, u.span.Attributes)
	}
}
...
func (p *Processor) upsertDimensions(prefix string, m map[string]string, resourceAttr, spanAttr []*v1_common.KeyValue) {
	...
	for _, dim := range p.dimensionLabels {
		if v, ok := processor_util.FindAttributeValue(dim.name, resourceAttr); ok {
		...
		v, ok := processor_util.FindAttributeValue(dim.name, spanAttr)
		if ok {
			if p.Cfg.EnableClientServerPrefix {
				if isClient {
					m[dim.clientName] = v
...
func (p *Processor) onCompleteAt(e *store.Edge, updateTimeMs int64) {
	...
	if usesBytesExemplar(p.Cfg.HistogramOverride) && e.TraceIDLen > 0 {
...
func (p *Processor) setEdgeTraceID(e *store.Edge, traceID []byte) {
	...
	if len(traceID) <= len(e.TraceIDRaw) && usesBytesExemplar(p.Cfg.HistogramOverride) {
...
// usesBytesExemplar reports whether the histogram observe path supports the
// raw-bytes exemplar form. The string-only legacy path is reserved for
// HistogramOverride values that don't go through one of our histograms.
func usesBytesExemplar(mode registry.HistogramMode) bool {
	switch mode {
	case registry.HistogramModeClassic, registry.HistogramModeNative, registry.HistogramModeBoth:
		return true
	}
	return false
}
...
func (p *Processor) addDroppedSpanSide(span *v1_trace.Span) {
	if isClient(span.Kind) {
		key := store.EncodeKey(span.TraceId, span.SpanId)
	...
	if isServer(span.Kind) {
		...
		key := store.EncodeKey(span.TraceId, span.ParentSpanId)
