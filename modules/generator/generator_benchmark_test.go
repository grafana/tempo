...
func BenchmarkRetainedMemoryProductionCardinality(b *testing.B) {
	for _, tc := range []struct {
		...
		b.Run(tc.name, func(b *testing.B) {
			...
			ctx := context.Background()
			// Each iteration seeds 100k series + reads memstats; the timed
			// section deliberately covers the whole iteration so the framework
			// can calibrate b.N from real wall time. The "ns/op" number reflects
			// that whole-iteration cost; the reported active_series and
			// heap_bytes/active_series metrics are the values of interest.
			for i := 0; i < b.N; i++ {
				runtime.GC()
