...
class MLTBaseModel(ABC):
    ...
    def load_pipeline_from_file(self, model_path: Union[Path, str]):
        ...
        for name, step in self._new_pipeline().named_steps.items():
            ...
            fail |= type(step) is not type(new_step)

        if fail:
            raise ValueError(
                "The steps in pipeline from the file do not match the steps for this model"
            )
    ...
    def get_fold_from_index(self, data: pd.DataFrame, indices: np.ndarray) -> pd.DataFrame:
        """
        Get the data at the passed indices plus any additional values that may be needed.
         Sequence models need a full
        sequence length before making predictions so their implementation of this
         method will prepend the necessary
        samples.
    ...
    def transform_data_to_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.true_label_name in features.columns:
            logging.warning(
                f"{self.true_label_name} is not in the passed `non_feature_columns`"
                " it will be removed but you should "
    ...
    def _match_data_to_features(self, data, features):
        """
        In sequential models `transform_data_to_features` may alter the shape of the data
         which causes issues because we
    ...
    def train_model(self, data: pd.DataFrame):
        ...
        """
        self.logger.info(
            "Model Training Start. Start date: "
            f"{data[self.prediction_time_column_name].min().strftime('%Y-%m-%d')}. "
    ...
    def _prepare_labels(self, label_series) -> np.ndarray:
        """
        Prepare labels for training and parameter tuning by turning them into a numpy
        array and encoding them if needed
    ...
    def tune_hyperparameters(self, data: pd.DataFrame):
        ...
        self.cv_set = [
            (dates.iloc[train_indices], dates.iloc[test_indices])
    ...
    def _trim_cv_params(params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Remove unneeded keys and values from search_cv params. These have the
        estimator as an object which we don't need
        another copy of.
    ...
    def get_predictions(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        :return DataFrame with predictions and prediction probabilities as
    ...
    def get_booster_if_needed(self) -> Any:
        ...
        :return: The booster for the model if pipeline["estimator"] has one
        otherwise just the estimator itself
        """
        try:
    ...
    def model_parameters(self):
        ...
        return {
            ...
            if key in self.model_parameter_grid
        }
...
class MLTBaseRegression(MLTBaseModel, ABC):
    ...
    def _get_prediction_data(self, features: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
        return pd.DataFrame(
            {self.predicted_label_name: self.pipeline.predict(features)}, index=index
        )
    ...
    def get_shap_values(self, feature_data: pd.DataFrame, predictions: pd.Series) -> pd.DataFrame:
        ...
        This currently works for LightGBM and XGBoost,
        but may require adjustments for other models.

        :param feature_data: Raw feature data before any selection or transformations,
        including metadata.
        ...
        # Compute SHAP values
        transformed_features = self.pipeline.named_steps.feature_selection.transform(
        ...
        used_names = self.pipeline.named_steps.feature_selection.get_feature_names_out(
            feature_names
        )

        # Assign SHAP values to the original dataframe
        feature_data[used_names] = shap_values
        feature_data[[x for x in feature_names if x not in used_names]] = (
            0  # Set SHAP values to 0 for unused features
        )
...
class MLTBaseClassification(MLTBaseModel, ABC):
    ...
    def _get_prediction_data(self, features: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
        predictions = pd.DataFrame(
            ...
            columns=self.probability_column_names,
            index=index,
        )
    ...
    def get_shap_values(self, feature_data: pd.DataFrame, predictions: pd.Series) -> pd.DataFrame:
        if self.model_binary:
            raise NotImplementedError(
                "SHAP values are not supported for binary classification models"
            )
        feature_names = [col for col in feature_data.columns if col not in self.non_feature_columns]
