...
class SampleWeightClassifier(xgb.XGBClassifier):
    ...
    def get_sample_weight(self, y: np.ndarray) -> np.ndarray:
        ...
        label_to_class_weight = {
            unique_labels[i]: class_weights[i] for i in range(len(unique_labels))
        }
...
class MLTXGBoostClassification(MLTBaseClassification):
    ...
    def __init__(
        ...
        if self.sampling_method is not None and not use_gpu:
            logging.warning(
                "Sampling method is not supported for CPU models. Setting sampling method to None."
            )
    ...
    def feature_importances(self, training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        importances = pd.DataFrame(list(raw_importances.items()), columns=["feature", "score"])

        return importances

    def get_shap_values(
        self, feature_data: pd.DataFrame, predictions: pd.DataFrame
    ) -> pd.DataFrame:
