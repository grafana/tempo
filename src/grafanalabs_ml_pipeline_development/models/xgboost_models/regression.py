...
class MLTXGBoostRegression(MLTBaseRegression):
    ...
    def __init__(
        ...
        if self.use_target_transform and self.target_transform not in valid_transforms:
    ...
    def _get_transformer(self):
        ...
        if self.target_transform in ("yeo-johnson", "box-cox"):
            return PowerTransformer(
                method=self.target_transform, standardize=self.target_standardize
            )
    ...
    def _new_pipeline(self):
        ...
        feature_pipeline = Pipeline(
            [
                (
                    "feature_selection",
                    SelectKBest(score_func=f_regression).set_output(transform="pandas"),
                ),
