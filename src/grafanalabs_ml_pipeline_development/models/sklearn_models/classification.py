...
class MLTSKLearnClassification(MLTBaseClassification):
    ...
    def feature_importances(self, training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        names = self.pipeline.name_steps.feature_selection.get_feature_names_out(
    ...
    def get_shap_values(
        ...
        shap_values = [
            (logistic_regressor.coef_[int(i), :] * transformed).to_numpy()
            for i in logistic_regressor.classes_
        ]
        expected_values = logistic_regressor.predict_proba(transformed)
        return self._make_scaled_shap_df(
            transformed.columns,
            feature_data[metadata_columns],
            predictions,
            shap_values,
            expected_values,
        )
