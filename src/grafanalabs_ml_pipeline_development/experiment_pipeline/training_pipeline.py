...
class PredictionResults:
    ...
    Attributes:
        out_of_sample_predictions (pd.DataFrame, optional): Out-of-sample
    ...
    def concat(cls, results: Iterable["PredictionResults"]) -> "PredictionResults":
        ...
        Args:
            results (Iterable[PredictionResults]): An iterable of
            PredictionResults objects to combine

        Returns:
            PredictionResults: A new PredictionResults object containing the
            concatenated data from all input results,
                with fields combined appropriately
        """
        results = sorted(results, key=lambda r: r.prediction_end_time)

        return PredictionResults(
            out_of_sample_predictions=cls._concat_df_field(
                results, lambda r: r.out_of_sample_predictions
            ),
            in_sample_predictions=cls._concat_df_field(
                results, lambda r: r.in_sample_predictions
    ...
    def _concat_df_field(
        results: Iterable["PredictionResults"],
        field_getter: Callable[["PredictionResults"], pd.DataFrame],
        ...
        Args:
            ...
            field_getter (Callable[[PredictionResults], pd.DataFrame]):
            Function to extract the DataFrame field from each result

        Returns:
            Union[None, pd.DataFrame]: Concatenated DataFrame if any non-None
            fields exist, otherwise None
        """
...
class TrainingPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            is_smoke_test (bool, optional): Flag for running abbreviated smoke test.
            Defaults to False.
        """
    ...
    def get_tune_task_blocks(
        self, data: pd.DataFrame
    ) -> Generator[List[PredictionTask], None, None]:
        """
    ...
    def run_prediction_task(model: MLTBaseModel, task: PredictionTask) -> PredictionResults:
        ...
        train_times_dict = dict(
            tune_start_time=task.min_train_time, tune_end_time=task.max_train_time
        )
        ...
        if model.has_feature_importance:
            ...
            feature_importance[f"{'tune' if task.is_tune_task else 'train'}_start_time"] = (
                task.min_train_time
            )
            feature_importance[f"{'tune' if task.is_tune_task else 'train'}_end_time"] = (
                task.max_train_time
            )
        ...
        in_sample_predictions = in_sample_predictions[
            [
                x
                for x in in_sample_predictions.columns
                if x in added_cols.union(model.non_feature_columns)
            ]
        ]
        ...
        if model.has_shap and prediction_end_time.strftime("%Y-%m-%d") > task.min_shap_date:
            shaps = model.get_shap_values(
                feature_data=task.prediction_data, predictions=predictions
            )
            results.shaps = shaps
    ...
    def generate_predictions_for_prediction_period(
        self, data_for_training: pd.DataFrame
    ) -> PredictionResults:
        """
        ...
        if self.num_outer_cv_jobs == 1:
            block_list = [
                run_task_block(b, self.prediction_model)
                for b in self.get_tune_task_blocks(data_for_training)
            ]
        else:
            block_list = Parallel(n_jobs=self.num_outer_cv_jobs)(
                delayed(run_task_block)(b, self.prediction_model)
                for b in self.get_tune_task_blocks(data_for_training)
            )
