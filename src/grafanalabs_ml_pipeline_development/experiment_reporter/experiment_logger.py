...
class ExperimentLogger(ABC):
    ...
    def default_save_experiment_artifacts_to_disk(
        ...
        """
        Save experiment artifacts to disk including predictions, feature importance,
        model parameters, and SHAP values.
        ...
        all_results : PredictionResults
            Results from all predictions including out-of-sample predictions,
        ...
        if all_results.feature_importances is not None:
            self.save_feature_importances(
                last_results.feature_importances, prediction_model.importance_type
            )
            self.save_feature_importances(
                all_results.feature_importances.groupby("feature")["score"]
                .sum()
                .to_frame()
                .reset_index(),
            ...
            all_results.feature_importances.to_csv(
                self.experiment_path / "historical_feature_importance.csv", index=False
            )

        self.perform_model_checks(predictions)
        self.save_cv_split(prediction_model.cv_set)
        predictions[self.prediction_columns_to_save].to_pickle(
            self.experiment_path / "historicalpredictions.pkl"
        )
        all_results.in_sample_predictions.to_pickle(
            self.experiment_path / "in_sample_predictions.pkl"
        )
        all_results.model_params.to_csv(
            self.experiment_path / "model_params_history.csv", index=False
        )
    ...
    def save_unlabelled_prediction_artifacts_to_disk(
        ...
        """
        unlabelled_predictions[self.prediction_columns_to_save].to_csv(
            self.experiment_path / "recent_unlabelled_predictions.csv", index=False
        )
        if unlabelled_shaps is not None:
            unlabelled_shaps.to_parquet(self.experiment_path / "recent_unlabelled_shaps.parquet")

    def save_metrics_over_time_plot(
        self, data: pd.DataFrame, metric_dict: Dict[str, ScoringFunction] = None
    ):
        ...
        for resampling_period, resampling_code in self.resampling_to_frequency_code.items():
            for metric_name, metric in metric_dict.items():
                print(metric_name)
                metric_by_class_and_period = self.find_metrics_by_period(
                    data, metric, resampling_code
                )
                ...
                plt.savefig(
                    self.metrics_save_path / f"{metric_name}-{resampling_period}.png",
                    bbox_inches="tight",
                )
    ...
    def get_ml_metrics(self, true_label: pd.Series, predicted_label: pd.Series) -> Dict[str, float]:
        ...
        """
        return {
            name: metric(true_label, predicted_label)
    ...
    def save_feature_importances(
        ...
        -----
        Saves two files:
        - CSV file with raw feature importance scores at {metrics_save_path}/
        feature_importances_{importance_type}.csv
        - Bar plot visualization of top 30 features at
        {metrics_save_path}/feature_importances.png
        """
        feature_importances = feature_importances.sort_values("score", ascending=False)
        feature_importances.to_csv(
            self.metrics_save_path / f"feature_importances_{importance_type}.csv"
        )
    ...
    def find_metrics_by_period(
        ...
        """
        metric_by_period = label_data.resample(
            resampling_code, on=self.prediction_time_column_name
        ).apply(lambda x: metric(x[self.true_label_name], x[self.predicted_label_name]))
    ...
    def save_cv_split(self, cv_set: List[Tuple[pd.Series, pd.Series]]):
        ...
        all_dates = np.sort(
            pd.concat(
                [cv_set[i][j] for i in range(len(cv_set)) for j in range(len(cv_set[0]))]
            ).unique()
        )
        ...
        ax.set(
            yticks=range(len(cv_set)),
            xlabel="Date",
            ylabel="CV iteration",
            ylim=(-0.5, len(cv_set) - 0.5),
        )
    ...
    def save_unlabelled_prediction_processed(
        ...
        # Save figure
        fig.savefig(
            self.metrics_save_path / f"unlabelled_predictions_overage_mrr_{date}.png",
            bbox_inches="tight",
        )
        plt.close(fig)
        ...
        table = ax.table(
            cellText=cell_text, colLabels=list(stats.keys()), loc="center", cellLoc="center"
        )
        ...
        # Save figure
        fig.savefig(
            self.metrics_save_path / f"prediction_statistics_post_processing_{date}.png",
            bbox_inches="tight",
        )
        plt.close(fig)

        unlabelled_predictions[self.prediction_columns_to_save].to_csv(
            self.experiment_path / "recent_unlabelled_predictions_post_processing.csv", index=False
        )
...
class RegressionExperimentLogger(ExperimentLogger):
    ...
    def save_fit_plot(self, data: pd.DataFrame):
        ...
        fit_df = pd.DataFrame(
            {
                "true_value": data[self.true_label_name],
                "predicted_value": data[self.predicted_label_name],
            }
        )
    ...
    def save_predicted_true_scatter_plot(self, data: pd.DataFrame, ols_line: bool = True):
        ...
        """
        """
        data = data.rename(columns={self.predicted_label_name:
        "predicted_value", self.true_label_name: "actual_value"})
        fig, _ = plot_scatter(
        ...
        data = data.rename(
            columns={
                self.predicted_label_name: "predicted_value",
                self.true_label_name: "actual_value",
            }
        )
        ...
        if ols_line:
            ...
            x_range = np.linspace(
                data["predicted_value"].min(), data["predicted_value"].max(), 100
            ).reshape(-1, 1)
    ...
    def perform_model_checks(self, data: pd.DataFrame):
        ...
        self.generate_and_save_ml_artifacts(
            data[self.true_label_name], data[self.predicted_label_name]
        )
...
class ClassificationExperimentLogger(ExperimentLogger):
    ...
    def save_confusion_matrix(self, true_label: pd.Series, predicted_label: pd.Series):
        ...
        for title, normalize in normalization_options:
            ...
            fig.savefig(
                self.metrics_save_path / f"confusion_matrix_{title}.png", bbox_inches="tight"
            )
            plt.close()
    ...
    def save_classification_report(self, true_label: pd.Series, predicted_label: pd.Series):
        ...
        pd.DataFrame(ml_report).trnaspose().to_csv(
            self.metrics_save_path / "classification_report.csv"
        )
    ...
    def find_metrics_by_period(
        ...
        metric_by_period = label_data.resample(
            ...
            lambda x: self.metric_helper(
                x[self.true_label_name], x[self.predicted_label_name], sum_axis
            )
        )
    ...
    def save_reliability_plots(self, data: pd.DataFrame):
        fig, ax = plt.subplots(
            1, len(self.labels), figsize=(15, 6), facecolor="w", edgecolor="k", sharey="all"
        )
    ...
    def save_certainty_histogram(self, data: pd.DataFrame):
        ...
        fig, ax = plt.subplots(
            num_classes, 1, figsize=(15, 15), facecolor="w", edgecolor="k", sharex="all"
        )
        min_win_prob = round(1 / num_classes, 1)
        bins = np.array(range(int(min_win_prob * 100), 105, 5)) / 100

        for i, label in enumerate(self.labels):
            win_probabilities = data.loc[
                data[self.true_label_name] == label, self.probability_column_names
            ].max(axis=1)
    ...
    def save_label_population_over_time(self, data: pd.DataFrame):
        data = data[
            [self.prediction_time_column_name, self.true_label_name, self.predicted_label_name]
        ].melt(
            id_vars=[self.prediction_time_column_name],
            var_name="label_type",
            value_name="applied_label",
        )
        original_labels = data.loc[
            data["label_type"] == self.ture_label_name, "applied_label"
        ].unique()
        predicted_labels = data.loc[
            data["label_type"] == self.predicted_label_name, "applied_label"
        ].unique()
        ...
        for freq_name, freq_code in self.resampling_to_frequency_code.items():
            ...
            transformed_label_data = (
                data.groupby(["frequency", "label_type", "applied_label"])[
                    self.prediction_time_column_name
                ]
                .count()
                .unstack([1, 2])
                .fillna(0)
            ...
            fig, ax = plt.subplots(
                2, 1, figsize=(15, 15), facecolor="w", edgecolor="k", sharex="col"
            )

            for ind, label_type in enumerate([self.true_label_name, self.predicted_label_name]):
                transformed_label_data[label_type][labels[ind]].rename(
                    dict(zip(self.labels, self.label_names))
                ).plot(
                    ax=ax[ind],
                )
            ...
            plt.savefig(
                self.metrics_save_path / f"label_population_over_time_{freq_name}.png",
                bbox_inches="tight",
            )
            plt.close()

    def perform_model_checks(self, data: pd.DataFrame):
        self.save_metrics_over_time_plot(
            data, metric_dict={"accuracy": recall_score, "precision": precision_score}
        )
