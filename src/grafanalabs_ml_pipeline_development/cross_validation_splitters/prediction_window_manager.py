...
class PredictionWindowManager(CVSplitter):
    ...
    def split(
        self, data: pd.DataFrame, labels: Optional[pd.Series] = None
        ...
        if self.initial_train_size > num_prediction_times:
            raise ValueError(
                f"Initial training size must be less than or equal to"
                " the number of prediction times. \n"
        ...
        prediction_window_starts = range(
            self.initial_train_size + self.gap_size,
            num_prediction_times,
            self.num_bars_between_training,
        )
        ...
        for prediction_start in prediction_window_starts:
            ...
            if self.rolling_window_size:
                train_times = prediction_times[
                    indices[max(train_end - self.rolling_window_size, 0) : train_end]
                ]
            else:
                train_times = prediction_times[indices[:train_end]]

            test_times = prediction_times[
                indices[prediction_start : prediction_start + self.num_bars_between_training]
            ]
            yield (
