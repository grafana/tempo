...
class BaseTimeSeriesCV(CVSplitter):
    ...
    def _check_split_arguments(self, X: pd.DataFrame, y: pd.Series = None):
        if not X[self.pred_time_column_name].is_monotonic_increasing:
            ...
            if len(non_monotonic_indices) > 0:
                ...
                error_msg = (
                    f"pred_time_column_name must be monotonic increasing."
                    " First violation at index {first_violation}:\n"
                    f"Value at index {first_violation}:"
                    " {X[self.pred_time_column_name].iloc[first_violation]}\n"
                    f"Value at index {first_violation + 1}:"
                    " {X[self.pred_time_column_name].iloc[first_violation + 1]}"
                )
        ...
        if y is not None and (X.index == y.index).sum() != len(X):
            raise ValueError("X and y must have the same index (mismatch found)")
    ...
    def embargo(
        ...
        if min_train_index < all_indices.shape[0]:
            allowed_indices = np.concatenate(
                [all_indices[:test_fold_end], all_indices[min_train_index:]]
            )
    ...
    def purge(
        ...
        train_indices_1 = np.intersect1d(
            train_indices, all_indices[eval_times < time_test_fold_start]
        )
        ...
        if test_fold_end < all_indices.shape[0]:
            ...
            train_indices_2 = np.intersect1d(
                train_indices_2, all_indices[pred_times >= eval_time_test_end]
            )
...
class PurgedWalkForwardCV(BaseTimeSeriesCV):
    ...
    This class implements walk-forward cross validation with purging and embargo to
    ...
    def __init__(
        ...
        super().__init__(
            n_splits, pred_time_column_name, eval_time_column_name, split_by_time=split_by_time
        )
        ...
        if n_test_splits <= 0 or n_test_splits >= self.n_splits - 1:
            raise ValueError(
                "n_test_splits must be greater than 0 and less than n_splits - 1, "
                f"got {n_test_splits}"
            )
        ...
        if min_train_splits <= 0 or min_train_splits >= self.n_splits - self.n_test_splits:
            raise ValueError(
                "min_train_splits must be greater than 0 and less "
                f"than n_splits - n_test_splits, got {min_train_splits}"
            )
        ...
        if max_train_splits <= 0 or max_train_splits > self.n_splits - self.n_test_splits:
            raise ValueError(
                "max_train_splits must be greater than 0 and less than "
                f"n_splits - n_test_splits, got {max_train_splits}"
            )
        if self.min_train_splits > max_train_splits:
            raise ValueError(
                "min_train_splits must be less than or equal to max_train_splits, "
                f"got {self.min_train_splits} and {max_train_splits}"
            )
    ...
    def _split(
        ...
        This method generates training and test splits according to the PurgedWalkForward
        CV scheme:
        - Training data comes strictly before test data
        - Training window size grows from min_train_splits up to max_train_splits
        - Test window size remains constant at n_test_splits
        - Overlapping samples between train and test are purged based on prediction
    ...
    def _compute_fold_bounds(self, X: pd.DataFrame) -> List[int]:
        ...
        if self.split_by_time:
            ...
            fold_bounds_times = [
                pred_times.iloc[0] + fold_time_span * n for n in range(self.n_splits)
            ]
