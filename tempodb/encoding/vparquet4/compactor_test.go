...
// readBackTraceIDs opens the given completed block and reads back every trace ID actually
// persisted to it, using the same rawIter path compaction itself reads inputs with.
func readBackTraceIDs(ctx context.Context, t testing.TB, meta *backend.BlockMeta, r backend.Reader) []common.ID {
	block := newBackendBlock(meta, r)
	iter, err := block.rawIter(ctx, newRowPool(1_000_000))
	require.NoError(t, err)
	defer iter.Close()
...
func TestCompact_MultiOutput_AttributionExact(t *testing.T) {
	...
	for i, meta := range newCompactedBlocks {
		...
		actualIDs := readBackTraceIDs(context.Background(), t, meta, r)
