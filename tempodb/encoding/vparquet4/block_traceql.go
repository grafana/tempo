	...
	for _, cond := range conditions {
		...
		if entry, ok := wellKnownColumnLookups[cond.Attribute.Name]; ok && entry.level != traceql.AttributeScopeResource {
			...
			if entry.typ == traceql.TypeInt &&
				...
				if pred != nil {
					var floatPred parquetquery.Predicate
					if operandType(cond.Operands) == traceql.TypeFloat {
						floatPred, err = createFloatPredicate(cond.Op, cond.Operands)
					} else {
						floatPred, err = createFloatPredicateFromInt(cond.Op, cond.Operands)
					}
					if err != nil {
						return nil, fmt.Errorf("creating predicate: %w", err)
					}
					...
					if floatPred == nil {
						...
						iters = append(iters,
							parquetquery.NewUnionIterator(
								DefinitionLevelResourceSpansILSSpan,
								[]parquetquery.Iterator{
									makeIter(entry.columnPath, pred, cond.Attribute.Name),
									makeIter(columnPathSpanAttrDouble, floatPred, cond.Attribute.Name),
								},
								nil,
							),
						)
