...
func rowGroupsFromFile(pf *parquet.File, opts common.SearchOptions) []parquet.RowGroup {
	...
	if opts.TotalPages > 0 {
		...
		if opts.StartPage > len(rgs) && opts.TotalPages > len(rgs) {
			opts.StartPage = len(rgs)
			opts.TotalPages = len(rgs)
			return rgs[opts.StartPage:opts.TotalPages]
		} else if opts.StartPage+opts.TotalPages > len(rgs) {
