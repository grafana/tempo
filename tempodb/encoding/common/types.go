...
func NewIDMap[T any](estimatedCount int) *IDMap[T] {
	return &IDMap[T]{
		m: make([]IDMapEntry[T], 0, estimatedCount),
	}
}

func (m *IDMap[T]) Set(id ID, val T) {
	m.m = append(m.m, IDMapEntry[T]{id, val})
}

func (m *IDMap[T]) Has(id ID) bool {
	_, found := slices.BinarySearchFunc(m.m, id, func(e IDMapEntry[T], id ID) int {
		return bytes.Compare(e.ID, id)
	})
	return found
}

func (m *IDMap[T]) Get(id ID) (T, bool) {
	i, found := slices.BinarySearchFunc(m.m, id, func(e IDMapEntry[T], id ID) int {
		return bytes.Compare(e.ID, id)
	})
	if found {
		return m.m[i].Entry, true
	}
...
func (m *IDMap[T]) EntriesSortedByID() []IDMapEntry[T] {
	// Copy and sort entries by ID
	entries := make([]IDMapEntry[T], len(m.m))
	copy(entries, m.m)
