...
func descendantOfUnion(lhs []traceql.Span, rhs []traceql.Span, invert bool, buffer []traceql.Span) []traceql.Span {
	...
	if unsafe.SliceData(lhs) != unsafe.SliceData(rhs) {//#nosec G103 -- Checking that two objects have the same address is memory-safe. TODO can they ever be different lengths?  // if these are pointing to the same slice, no reason to sort again
