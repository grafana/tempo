...
func TestFetchTagValues(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("tag: %s, query: %s", tc.tag, tc.query), func(t *testing.T) {
			...
			autocompleteReq.ConditionGroups[0] = append(autocompleteReq.ConditionGroups[0], traceql.Condition{
				Attribute: tag,
				Op:        traceql.OpNone,
			})
			mc := collector.NewMetricsCollector()
...
func TestFetchTagNamesWithOrConditions(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                   "both sides match the same values",
			query:                  `{span.http.method="method-01-01" || span.generic-01-01="foo"}`,
			expectedSpanValues:     []string{"generic-01-01", "span-same"},
			expectedResourceValues: []string{"generic-01", "resource-same"},
		},
		{
			name:                   "both sides match different values",
			query:                  `{span.generic-01-01="foo" || span.generic-01-02="foo"}`,
			expectedSpanValues:     []string{"generic-01-01", "generic-01-02", "span-same"},
			expectedResourceValues: []string{"generic-01", "resource-same"},
		},
		{
			name:                   "three conditions match different values",
			query:                  `{span.generic-01-01="foo" || span.generic-01-02="foo" || span.generic-02-01="foo"}`,
		...
		},
		{
			...
			query:                  `{(resource.generic-01 = "bar" && span.generic-01-01="foo") || (resource.generic-01 = "bar" && span.generic-01-02="foo")}`,
			expectedSpanValues:     []string{"generic-01-01", "generic-01-02", "span-same"},
			expectedResourceValues: []string{"generic-01", "resource-same"},
		},
		{
			name:                   "both sides with double conditions match different values",
			query:                  `{(resource.generic-01 = "bar" || resource.generic-02 = "bar") &&  (span.generic-01-01="foo" || span.generic-01-02="foo" || span.generic-02-01="foo")}`,
			expectedSpanValues:     []string{"generic-01-01", "generic-01-02", "generic-02-01", "span-same"},
			expectedResourceValues: []string{"generic-01", "generic-02", "resource-same"},
		},
	}
	...
	for _, tc := range testCases {
		for _, scope := range []traceql.AttributeScope{traceql.AttributeScopeSpan, traceql.AttributeScopeResource, traceql.AttributeScopeNone} {
			...
			t.Run(fmt.Sprintf("%s-%s", tc.name, scope), func(t *testing.T) {
				distinctAttrNames := collector.NewScopedDistinctString(0, 0, 0)
				conditions, _ := traceql.ExtractConditionGroups(tc.query, traceql.DefaultMaxConditionGroups)
...
func TestFetchTagValuesWithOrConditions(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			tag:            "span.foo",
			query:          `{ resource.namespace="namespace3" || (resource.foo = "abc" && resource.asdf = 1234) }`,
			expectedValues: []tempopb.TagValue{stringTagValue("def")},
		},
		{
			name:           "both sides with different ops match",
			tag:            "span.foo",
			query:          `{ (resource.foo = "abc" && resource.asdf = 1234) || resource.foo = "abc2" }`,
			expectedValues: []tempopb.TagValue{stringTagValue("def"), stringTagValue("ghi")},
		},
		{
