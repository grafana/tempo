...
func createPredicate(op traceql.Operator, operands traceql.Operands) (parquetquery.Predicate, error) {
	...
	default:
		return nil, fmt.Errorf("cannot create predicate for operand: %s", operands[0].EncodeToString(false))
	}
}
...
func (c *spanCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, kv := range res.Entries {
		...
		case columnPathSpanID:
			...
			sp.attributes[traceql.IntrinsicSpanIDAttribute] = traceql.NewStaticString(util.IDToTrimmedHexString(kv.Value.ByteArray()))
