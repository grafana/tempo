...
func (rw *readerWriter) SearchTags(ctx context.Context, meta *backend.BlockMeta, scope string, opts common.SearchOptions) (*tempopb.SearchTagsV2Response, error) {
	...
	logger := log.WithContext(ctx, log.Logger)
	distinctValues := collector.NewScopedDistinctString(0, opts.Limit, opts.StaleValueThreshold, logger)
...
func (rw *readerWriter) SearchTagValues(ctx context.Context, meta *backend.BlockMeta, tag string, opts common.SearchOptions) (response *tempopb.SearchTagValuesResponse, err error) {
	...
	logger := log.WithContext(ctx, log.Logger)
	dv := collector.NewDistinctString(0, opts.Limit, opts.StaleValueThreshold, logger)
	mc := collector.NewMetricsCollector()
	rw.cfg.Search.ApplyToOptions(&opts)
...
func (rw *readerWriter) SearchTagValuesV2(ctx context.Context, meta *backend.BlockMeta, req *tempopb.SearchTagValuesRequest, opts common.SearchOptions) (*tempopb.SearchTagValuesV2Response, error) {
	...
	logger := log.WithContext(ctx, log.Logger)
	dv := collector.NewDistinctValue[tempopb.TagValue](0, opts.Limit, opts.StaleValueThreshold, func(v tempopb.TagValue) int { return len(v.Type) + len(v.Value) }, logger)
	mc := collector.NewMetricsCollector()
	rw.cfg.Search.ApplyToOptions(&opts)
