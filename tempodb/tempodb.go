...
func (rw *readerWriter) Find(ctx context.Context, tenantID string, id common.ID, blockStart string, blockEnd string) ([][]byte, []string, []error, error) {
	...
	curTime := time.Now()
	partialTraces, dataEncodings, blockErrs, err := rw.pool.RunJobs(ctx, copiedBlocklist, func(ctx context.Context, payload interface{}) ([]byte, string, error) {
	...
	return partialTraces, dataEncodings, blockErrs, err
}
