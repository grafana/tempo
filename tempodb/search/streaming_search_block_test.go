...
func newStreamingSearchBlockWithTraces(t testing.TB, traceCount int, enc backend.Encoding) (string, *StreamingSearchBlock) {
	...
	sb, err := NewStreamingSearchBlockForFile(f, uuid.New(), enc)
	require.NoError(t, err)
...
func TestStreamingSearchBlock(t *testing.T) {
	...
	var gotTags []string
	require.NoError(t, b1.Tags(ctx, func(k string) bool { gotTags = append(gotTags, k); return false }))
	sort.Strings(gotTags)
	require.Equal(t, tags, gotTags)

	for k, v := range tagValues {
		var gotValues []string
		require.NoError(t, b1.TagValues(ctx, k, func(s string) bool { gotValues = append(gotValues, s); return false }))
