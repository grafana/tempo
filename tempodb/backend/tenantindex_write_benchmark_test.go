			...
			alter := func(m *BlockMeta) {
				if strings.HasPrefix(tc.name, "name32") || strings.HasPrefix(tc.name, "name256") {
					...
					for i := range m.DedicatedColumns {
						name := m.DedicatedColumns[i].Name
						m.DedicatedColumns[i].Name = name + strings.Repeat("x", limit-len(name)-len(tc.suffix)) + tc.suffix
					}
					return
				}
				if tc.single {
					m.DedicatedColumns = m.DedicatedColumns[len(m.DedicatedColumns)-1:]
				}
				if tc.suffix != "" {
					if tc.all {
						for i := range m.DedicatedColumns {
							m.DedicatedColumns[i].Name += tc.suffix
						}
					} else {
						m.DedicatedColumns[len(m.DedicatedColumns)-1].Name += tc.suffix
					}
				}
			}
