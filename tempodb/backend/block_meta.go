...
type BlockMeta struct {
	...
	MaxSpanIndexAge time.Duration `json:"max_span_index_age"` // Maximum span index age that will adjust the StartTime of the BlockMeta
}
...
func (b *BlockMeta) ObjectAdded(id []byte, start uint32, end uint32) {
	...
	// We want to adjust the block meta time window only when the object being
	// added is not older than the MaxSpanIndexAge.  This will mean search
	// results will not be able to search for this trace.
	maxSpanAgeTime := time.Now().Add(-b.MaxSpanIndexAge)
	spanExtendsBlockTime := startTime.After(maxSpanAgeTime)

	if b.StartTime.IsZero() || (startTime.Before(b.StartTime) && spanExtendsBlockTime) {
		b.StartTime = startTime
	}
	if b.EndTime.IsZero() || (endTime.After(b.EndTime) && spanExtendsBlockTime) {
		b.EndTime = endTime
	}
