...
func TestCompactedBlockMetaJSONProtoRoundTrip(t *testing.T) {
	timeParse := func(s string) time.Time {
		date, err := time.Parse(time.RFC3339Nano, s)
		require.NoError(t, err)
		return date
	...
	compacted := CompactedBlockMeta{
		...
		CompactedTime: timeParse("2021-01-03T00:00:00.0000000Z"),
	}

	// block_meta is a regular (non-embedded) proto field, but CompactedBlockMeta must
	// still serialize BlockMeta's fields flattened to the top level alongside
	// compactedTime. This is the historical on-disk layout that gogoproto.embed produced
	// and that tenant index files depend on, so it must stay byte-stable across versions.
	expectedJSON := `{
    	"format": "vParquet3",
    	"blockID": "00000000-0000-0000-0000-000000000000",
    	"tenantID": "single-tenant",
	...
	// JSON. Marshal a pointer: CompactedBlockMeta.MarshalJSON has a pointer receiver, and
	// tenant index files hold []*CompactedBlockMeta, so this matches the real path.
	metaJSON, err := json.Marshal(&compacted)
	require.NoError(t, err)
	assert.JSONEq(t, expectedJSON, string(metaJSON))

	var metaRoundtrip CompactedBlockMeta
	require.NoError(t, json.Unmarshal(metaJSON, &metaRoundtrip))
	assert.Equal(t, compacted, metaRoundtrip)

	// proto
	protoData, err := compacted.Marshal()
	require.NoError(t, err)
	newMeta := CompactedBlockMeta{}
	require.NoError(t, newMeta.Unmarshal(protoData))

	assert.Equal(t, compacted, newMeta)
}
