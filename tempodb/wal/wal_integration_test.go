...
func TestIntegrationInvalidFilesAndFoldersAreHandled(t *testing.T) {
	tempDir := t.TempDir()
	wal, err := New(&Config{
		Filepath: tempDir,
...
func TestIntegrationAppendBlockStartEnd(t *testing.T) {
...
func TestIntegrationIngestionSlack(t *testing.T) {
	encodings := []encoding.VersionedEncoding{
		v2.Encoding{},
		vparquet.Encoding{},
	}
...
func TestIntregationFindByTraceID(t *testing.T) { // jpe - split these out?
	for _, e := range encoding.AllEncodings() {
		t.Run(e.Version(), func(t *testing.T) {
...
func TestIntegrationIterator(t *testing.T) {
	for _, e := range encoding.AllEncodings() {
		t.Run(e.Version(), func(t *testing.T) {
			testIterator(t, e)
		})
