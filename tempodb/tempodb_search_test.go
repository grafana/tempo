...
func tagValuesRunner(t *testing.T, _ *tempopb.Trace, _ *tempopb.TraceSearchMetadata, _, _ []*tempopb.SearchRequest, _ *backend.BlockMeta, _ Reader, bb common.BackendBlock) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			conditionGroups, _ := traceql.ExtractConditionGroups(tc.query, traceql.DefaultMaxConditionGroups)
...
func tagNamesRunner(t *testing.T, _ *tempopb.Trace, _ *tempopb.TraceSearchMetadata, _, _ []*tempopb.SearchRequest, bm *backend.BlockMeta, _ Reader, bb common.BackendBlock) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			conditionGroups, _ := traceql.ExtractConditionGroups(tc.query, traceql.DefaultMaxConditionGroups)
			err := e.ExecuteTagNames(ctx, traceql.AttributeScopeFromString(tc.scope), conditionGroups, func(tag string, scope traceql.AttributeScope) bool {
...
func TestSearchForTagsAndTagValues(t *testing.T) {
	...
	conditionGroups, _ := traceql.ExtractConditionGroups(`{resource.service.name="test-service-2"}`, traceql.DefaultMaxConditionGroups)
