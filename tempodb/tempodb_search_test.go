...
func tagValuesRunner(t *testing.T, _ *tempopb.Trace, _ *tempopb.TraceSearchMetadata, _, _ []*tempopb.SearchRequest, _ *backend.BlockMeta, _ Reader, bb common.BackendBlock) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := e.ExecuteTagValues(ctx, tc.tag, traceql.ExtractConditions(tc.query), traceql.MakeCollectTagValueFunc(valueCollector.Collect), fetcher)
			if errors.Is(err, util.ErrUnsupported) {
				return
			}
...
func tagNamesRunner(t *testing.T, _ *tempopb.Trace, _ *tempopb.TraceSearchMetadata, _, _ []*tempopb.SearchRequest, bm *backend.BlockMeta, _ Reader, bb common.BackendBlock) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := e.ExecuteTagNames(ctx, traceql.AttributeScopeFromString(tc.scope), traceql.ExtractConditions(tc.query), func(tag string, scope traceql.AttributeScope) bool {
...
func TestSearchForTagsAndTagValues(t *testing.T) {
	...
	err = traceql.NewEngine().ExecuteTagValues(context.Background(), tag, traceql.ExtractConditions(`{resource.service.name="test-service-2"}`), traceql.MakeCollectTagValueFunc(valueCollector.Collect), f)
	require.NoError(t, err)
