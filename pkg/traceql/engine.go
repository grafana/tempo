...
// Compile compiles given query. It does not support math expressions.
func Compile(query string) (*RootExpr, Pipeline, SpansetFilterFunc, *FetchSpansRequest, error) {
...
func CompileFetchSpanRequests(query string) (*RootExpr, map[string]FetchSpansRequest, error) {
	expr, err := Parse(query)
	if err != nil {
		return nil, nil, err
	}
...
func (e *Engine) ExecuteSearch(ctx context.Context, searchReq *tempopb.SearchRequest, fetcher SpansetFetcher, allowUnsafeQueryHints bool) (*tempopb.SearchResponse, error) {
	...
	rootExpr, pipeline, eval, fetchSpansRequest, err := Compile(searchReq.Query)
	if err != nil {
		return nil, err
	}
