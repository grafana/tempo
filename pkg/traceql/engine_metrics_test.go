...
import (
	...
	commonv1proto "github.com/grafana/tempo/v3/pkg/tempopb/common/v1"
...
func TestObserveSeriesAverageOverTimeForSpanAttribute(t *testing.T) {
	...
	e := NewEngine()
	layer1A, _ := e.CompileMetricsQueryRange(req)
	layer1B, _ := e.CompileMetricsQueryRange(req)
...
func processLayer1AndLayer2(req *tempopb.QueryRangeRequest, in ...[]Span) (SeriesSet, error) {
	...
	for _, spanSet := range in {
		layer1, err := e.CompileMetricsQueryRange(req)
		if err != nil {
			return nil, err
		}
