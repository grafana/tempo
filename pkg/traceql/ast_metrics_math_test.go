...
// MEDIUM-3 / TRQL-T-13: Deduplicated same-key sub-queries produce correct results.
// Regression test for the processLeaf in-place mutation bug: when both leaves share the
// same fragment key, the first processLeaf call must not strip labels from the shared
// input entries, so the second leaf can still find its series.
func TestMathExpressionSameKeyDedup(t *testing.T) {
	...
	for _, ts := range result {
		...
		for _, l := range ts.Labels {
			require.NotEqual(t, internalLabelQueryFragment, l.Name, "__query_fragment should be stripped")
			// Note: __name__ is re-added by the binary node with the combined name "(rate / rate)";
			// we only verify __query_fragment is stripped.
		}
	}
}

// MEDIUM-4 / TRQL-T-14: applyBinaryOp with multi-key rhs and scalar lhs does not panic.
// Regression test for the buffer pre-allocation under-count bug: when the scalar lhs
// has fewer values than a target series, offset must advance by the actual n, not by
// len(target[k].Values), to avoid an index-out-of-bounds on the next iteration.
func TestApplyBinaryOpMultiKeyScalarLhs(t *testing.T) {
	// lhs: scalar (no labels), 2 values
	// rhs: two keyed series each with 3 values
	// expected: 2 result series each with 2 values (min of 2 and 3)
	noLabels := Labels{}
	noLabelsKey := noLabels.MapKey()
	...
	// Should not panic; result has 2 series with 2 values each (min(2,3)=2)
	result := applyBinaryOp(OpDiv, lhs, rhs)
...
// Exercises the result() path which was previously at 0% coverage.
func TestBatchSeriesProcessorResult(t *testing.T) {
	// Use a realSeriesProcessor that returns real series
	...
	// Expect 2 series total, each tagged with __query_fragment
	...
	// Verify that series from different fragments are both present and keyed correctly
	// (they should not collide, since they have different service labels)
	for _, ts := range result {
		require.NotEmpty(t, ts.Values, "result series must have values")
	}
}

// Also verify unknown fragment is discarded (from TestBatchSeriesProcessorRoutes)
func TestBatchSeriesProcessorDiscardUnknown(t *testing.T) {
	...
	bsp.observeSeries([]*tempopb.TimeSeries{
		makePTS("frag-A", "svc1"),
		makePTS("frag-B", "svc2"),
		makePTS("unknown", "svc3"), // must be silently discarded
	})

	// 2 known series delivered to processors; unknown is silently discarded (not 3)
