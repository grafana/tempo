...
// NewMetricsAggregate creates a MetricsAggregate for use outside the traceql package.
func NewMetricsAggregate(op MetricsAggregateOp, by []Attribute) *MetricsAggregate {
	return newMetricsAggregate(op, by)
}

// Op returns the aggregation operation.
func (a *MetricsAggregate) Op() MetricsAggregateOp { return a.op }

// By returns the group-by attributes.
func (a *MetricsAggregate) By() []Attribute { return a.by }

// Init initialises the aggregator for the given request and mode.
func (a *MetricsAggregate) Init(req *tempopb.QueryRangeRequest, mode AggregateMode) {
	a.init(req, mode)
}

// Observe records a single span in the aggregator.
func (a *MetricsAggregate) Observe(s Span) { a.observe(s) }

// ObserveExemplar records an exemplar for a span.
func (a *MetricsAggregate) ObserveExemplar(s Span) { a.observeExemplar(s) }

// Result returns the accumulated SeriesSet, scaled by multiplier.
func (a *MetricsAggregate) Result(multiplier float64) SeriesSet { return a.result(multiplier) }

// Length returns the number of distinct series accumulated so far.
func (a *MetricsAggregate) Length() int { return a.length() }
