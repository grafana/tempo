...
// RedactionPhase reports why a batch is still present, which is what separates "still
// deleting" from "finished deleting but not yet torn down".
type RedactionPhase int32

const (
	RedactionPhase_REDACTION_PHASE_UNSPECIFIED RedactionPhase = 0
	// Block jobs are still outstanding.
	RedactionPhase_REDACTION_PHASE_RUNNING RedactionPhase = 1
	// All block jobs finished; waiting on the rescan deadline to cover blocks that compaction
	// produced from inputs skipped at submission.
	RedactionPhase_REDACTION_PHASE_AWAITING_RESCAN RedactionPhase = 2
	// All block jobs finished and no rescan is pending. The batch is held open only to keep
	// compaction disabled until removal.
	RedactionPhase_REDACTION_PHASE_QUIESCING RedactionPhase = 3
)

var RedactionPhase_name = map[int32]string{
	0: "REDACTION_PHASE_UNSPECIFIED",
	1: "REDACTION_PHASE_RUNNING",
	2: "REDACTION_PHASE_AWAITING_RESCAN",
	3: "REDACTION_PHASE_QUIESCING",
}

var RedactionPhase_value = map[string]int32{
	"REDACTION_PHASE_UNSPECIFIED":     0,
	"REDACTION_PHASE_RUNNING":         1,
	"REDACTION_PHASE_AWAITING_RESCAN": 2,
	"REDACTION_PHASE_QUIESCING":       3,
}

func (x RedactionPhase) String() string {
	return proto.EnumName(RedactionPhase_name, int32(x))
}

func (RedactionPhase) EnumDescriptor() ([]byte, []int) {
...
// GetRedactionStatusRequest asks for the state of the authenticated tenant's redaction batch.
// The tenant is sourced exclusively from the X-Scope-OrgID request header; the message carries
// no tenant field, so there is nothing to spoof.
type GetRedactionStatusRequest struct {
}

func (m *GetRedactionStatusRequest) Reset()         { *m = GetRedactionStatusRequest{} }
func (m *GetRedactionStatusRequest) String() string { return proto.CompactTextString(m) }
func (*GetRedactionStatusRequest) ProtoMessage()    {}
func (*GetRedactionStatusRequest) Descriptor() ([]byte, []int) {
	return fileDescriptor_1e9b87dd365f5504, []int{11}
}
func (m *GetRedactionStatusRequest) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *GetRedactionStatusRequest) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_GetRedactionStatusRequest.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *GetRedactionStatusRequest) XXX_Merge(src proto.Message) {
	xxx_messageInfo_GetRedactionStatusRequest.Merge(m, src)
}
func (m *GetRedactionStatusRequest) XXX_Size() int {
	return m.Size()
}
func (m *GetRedactionStatusRequest) XXX_DiscardUnknown() {
	xxx_messageInfo_GetRedactionStatusRequest.DiscardUnknown(m)
}

var xxx_messageInfo_GetRedactionStatusRequest proto.InternalMessageInfo

// GetRedactionStatusResponse describes the tenant's current redaction batch, or reports that
// there is none. A finished redaction leaves no batch behind, so "no batch" is the normal
// terminal state; the durable record of what a redaction removed is the
// redaction_traces_found_total metric, not this RPC.
type GetRedactionStatusResponse struct {
	// active reports whether a batch exists for the tenant. Every other field is unset when false.
	Active            bool           `protobuf:"varint,1,opt,name=active,proto3" json:"active,omitempty"`
	BatchId           string         `protobuf:"bytes,2,opt,name=batch_id,json=batchId,proto3" json:"batch_id,omitempty"`
	Mode              RedactionMode  `protobuf:"varint,3,opt,name=mode,proto3,enum=tempopb.RedactionMode" json:"mode,omitempty"`
	Phase             RedactionPhase `protobuf:"varint,4,opt,name=phase,proto3,enum=tempopb.RedactionPhase" json:"phase,omitempty"`
	...
	// The window frozen at submission. 0 means unbounded on that side.
	...
	EndTimeUnixNano   int64 `protobuf:"varint,7,opt,name=end_time_unix_nano,json=endTimeUnixNano,proto3" json:"end_time_unix_nano,omitempty"`
	// jobs_created is the block-job count recorded at submission: the denominator for progress.
	// A batch persisted by a binary predating the field reports 0, which is indistinguishable
	// from an empty batch -- submission refuses an empty selection, so read 0 as "unknown".
	JobsCreated int32 `protobuf:"varint,8,opt,name=jobs_created,json=jobsCreated,proto3" json:"jobs_created,omitempty"`
	// jobs_remaining counts block jobs still queued, in flight between the queue and a worker, or
	// running. Progress is jobs_created - jobs_remaining rather than a count of completions,
	// because Prune deletes terminal jobs while a long batch is still going.
	JobsRemaining int32 `protobuf:"varint,9,opt,name=jobs_remaining,json=jobsRemaining,proto3" json:"jobs_remaining,omitempty"`
	// jobs_running is the subset of jobs_remaining currently assigned to a worker. It separates a
	// batch that is progressing from one starved of workers.
	JobsRunning int32 `protobuf:"varint,10,opt,name=jobs_running,json=jobsRunning,proto3" json:"jobs_running,omitempty"`
	// jobs_failed counts block jobs that ended in failure and have not yet been pruned. Non-zero
	// means the redaction left blocks uncovered. Best-effort, for the same pruning reason.
	JobsFailed int32 `protobuf:"varint,11,opt,name=jobs_failed,json=jobsFailed,proto3" json:"jobs_failed,omitempty"`
}

func (m *GetRedactionStatusResponse) Reset()         { *m = GetRedactionStatusResponse{} }
func (m *GetRedactionStatusResponse) String() string { return proto.CompactTextString(m) }
func (*GetRedactionStatusResponse) ProtoMessage()    {}
func (*GetRedactionStatusResponse) Descriptor() ([]byte, []int) {
	return fileDescriptor_1e9b87dd365f5504, []int{12}
}
func (m *GetRedactionStatusResponse) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *GetRedactionStatusResponse) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_GetRedactionStatusResponse.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *GetRedactionStatusResponse) XXX_Merge(src proto.Message) {
	xxx_messageInfo_GetRedactionStatusResponse.Merge(m, src)
}
func (m *GetRedactionStatusResponse) XXX_Size() int {
	return m.Size()
}
func (m *GetRedactionStatusResponse) XXX_DiscardUnknown() {
	xxx_messageInfo_GetRedactionStatusResponse.DiscardUnknown(m)
}

var xxx_messageInfo_GetRedactionStatusResponse proto.InternalMessageInfo

func (m *GetRedactionStatusResponse) GetActive() bool {
	if m != nil {
		return m.Active
	}
	return false
...
func (m *GetRedactionStatusResponse) GetBatchId() string {
	if m != nil {
		return m.BatchId
	}
	return ""
...
func (m *GetRedactionStatusResponse) GetMode() RedactionMode {
	if m != nil {
		return m.Mode
	}
	return RedactionMode_REDACTION_MODE_APPLY
...
func (m *GetRedactionStatusResponse) GetPhase() RedactionPhase {
	if m != nil {
		return m.Phase
	}
	return RedactionPhase_REDACTION_PHASE_UNSPECIFIED
}

func (m *GetRedactionStatusResponse) GetCreatedAtUnixNano() int64 {
	if m != nil {
...
func (m *GetRedactionStatusResponse) GetStartTimeUnixNano() int64 {
	if m != nil {
		return m.StartTimeUnixNano
	}
	return 0
...
func (m *GetRedactionStatusResponse) GetEndTimeUnixNano() int64 {
	if m != nil {
		return m.EndTimeUnixNano
	}
	return 0
...
func (m *GetRedactionStatusResponse) GetJobsCreated() int32 {
	if m != nil {
		return m.JobsCreated
	}
	return 0
...
func (m *GetRedactionStatusResponse) GetJobsRemaining() int32 {
	if m != nil {
		return m.JobsRemaining
	}
	return 0
}

func (m *GetRedactionStatusResponse) GetJobsRunning() int32 {
	if m != nil {
...
func (m *GetRedactionStatusResponse) GetJobsFailed() int32 {
	if m != nil {
		return m.JobsFailed
	}
	return 0
...
type RedactionBatch struct {
	...
	// jobs_created is the number of block jobs enqueued for this batch at submission. Stored so
	// progress reporting has a denominator that does not decay: Prune deletes terminal jobs after
	// PruneAge, so counting surviving completions understates a long-running batch.
	JobsCreated int32 `protobuf:"varint,12,opt,name=jobs_created,json=jobsCreated,proto3" json:"jobs_created,omitempty"`
}
...
func (m *RedactionBatch) GetJobsCreated() int32 {
	if m != nil {
		return m.JobsCreated
	}
	return 0
...
func init() {
	...
	proto.RegisterEnum("tempopb.RedactionPhase", RedactionPhase_name, RedactionPhase_value)
	...
	proto.RegisterType((*GetRedactionStatusRequest)(nil), "tempopb.GetRedactionStatusRequest")
	proto.RegisterType((*GetRedactionStatusResponse)(nil), "tempopb.GetRedactionStatusResponse")
...
var fileDescriptor_1e9b87dd365f5504 = []byte{
	...
	,
}
...
type BackendSchedulerClient interface {
	...
	// Report the state of the authenticated tenant's current redaction batch. Read-only.
	GetRedactionStatus(ctx context.Context, in *GetRedactionStatusRequest, opts ...grpc.CallOption) (*GetRedactionStatusResponse, error)
}
...
func (c *backendSchedulerClient) GetRedactionStatus(ctx context.Context, in *GetRedactionStatusRequest, opts ...grpc.CallOption) (*GetRedactionStatusResponse, error) {
	out := new(GetRedactionStatusResponse)
	err := c.cc.Invoke(ctx, "/tempopb.BackendScheduler/GetRedactionStatus", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type BackendSchedulerServer interface {
	...
	// Report the state of the authenticated tenant's current redaction batch. Read-only.
	GetRedactionStatus(context.Context, *GetRedactionStatusRequest) (*GetRedactionStatusResponse, error)
}
...
func (*UnimplementedBackendSchedulerServer) GetRedactionStatus(ctx context.Context, req *GetRedactionStatusRequest) (*GetRedactionStatusResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetRedactionStatus not implemented")
}
...
func _BackendScheduler_GetRedactionStatus_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetRedactionStatusRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BackendSchedulerServer).GetRedactionStatus(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/tempopb.BackendScheduler/GetRedactionStatus",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BackendSchedulerServer).GetRedactionStatus(ctx, req.(*GetRedactionStatusRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var _BackendScheduler_serviceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "GetRedactionStatus",
			Handler:    _BackendScheduler_GetRedactionStatus_Handler,
		},
	},
...
func (m *GetRedactionStatusRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *GetRedactionStatusRequest) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *GetRedactionStatusRequest) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *GetRedactionStatusResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *GetRedactionStatusResponse) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *GetRedactionStatusResponse) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.JobsFailed != 0 {
		i = encodeVarintBackendwork(dAtA, i, uint64(m.JobsFailed))
		i--
		dAtA[i] = 0x58
	}
	if m.JobsRunning != 0 {
		i = encodeVarintBackendwork(dAtA, i, uint64(m.JobsRunning))
		i--
		dAtA[i] = 0x50
	}
	if m.JobsRemaining != 0 {
		i = encodeVarintBackendwork(dAtA, i, uint64(m.JobsRemaining))
		i--
		dAtA[i] = 0x48
	}
	if m.JobsCreated != 0 {
	...
	if m.Phase != 0 {
		i = encodeVarintBackendwork(dAtA, i, uint64(m.Phase))
		i--
		dAtA[i] = 0x20
	}
	if m.Mode != 0 {
...
func (m *GetRedactionStatusRequest) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *GetRedactionStatusResponse) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if m.Phase != 0 {
		n += 1 + sovBackendwork(uint64(m.Phase))
	}
	if m.CreatedAtUnixNano != 0 {
	...
	if m.JobsRemaining != 0 {
		n += 1 + sovBackendwork(uint64(m.JobsRemaining))
	}
	if m.JobsRunning != 0 {
		n += 1 + sovBackendwork(uint64(m.JobsRunning))
	}
	if m.JobsFailed != 0 {
		n += 1 + sovBackendwork(uint64(m.JobsFailed))
	}
	return n
}
...
func (m *GetRedactionStatusRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: GetRedactionStatusRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: GetRedactionStatusRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
...
func (m *GetRedactionStatusResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: GetRedactionStatusResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: GetRedactionStatusResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 4:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Phase |= RedactionPhase(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 9:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field JobsRemaining", wireType)
			}
			m.JobsRemaining = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.JobsRemaining |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 10:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.JobsRunning |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 11:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.JobsFailed |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
