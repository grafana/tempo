...
// todo: this file borrowed from the opentelemetry collector (translator/trace/oc_to_otlp.go) to do trace translation
//  hopefully these methods will be exposed as a consumable package in the near future
...
func timestampToUnixnano(ts *timestamp.Timestamp) uint64 {
	...
	return uint64(time.Unix(ts.Seconds, int64(ts.Nanos)).UnixNano())
}
