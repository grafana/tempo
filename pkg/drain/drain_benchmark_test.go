...
func BenchmarkDrain_DefaultConfigSteadyState(b *testing.B) {
	...
	b.ReportMetric(float64(len(d.exactLocationByClusterID)), "indexed_clusters")
}
...
func BenchmarkDrain_PruneMaxClusters(b *testing.B) {
	...
	leaf.exactClusterIDByPattern = make(map[string]int, cfg.MaxClusters)
	d.rootNode.keyToChildNode["2"] = leaf

	for i := 1; i <= cfg.MaxClusters; i++ {
		pattern := "GET" + strconv.Itoa(i)
		cluster := &LogCluster{
			id:          i,
			Tokens:      []string{pattern, "<END>"},
			Stringer:    d.tokenizer.Join,
			ParamString: cfg.ParamString,
		}
		d.idToCluster.Put(cluster)
		leaf.clusterIDs = append(leaf.clusterIDs, i)
		leaf.exactClusterIDByPattern[pattern] = i
		d.exactLocationByClusterID[i] = exactLocation{node: leaf, pattern: pattern}
	}
