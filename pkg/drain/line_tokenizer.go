...
func (t *DefaultTokenizer) Tokenize(line string, tokens []string, _ any, _ *prometheus.CounterVec) ([]string, any) {
	tokens = tokens[:0]
...
func (t *DefaultTokenizer) Join(tokens []string, state any) string {
	// Last token is always <END> so we don't need to include it.
	return strings.Join(tokens[0:len(tokens)-1], "")
}

func (t *DefaultTokenizer) Clone(tokens []string, state any) ([]string, any) {
	return slices.Clone(tokens), state
}
