...
func TestScopedDistinct(t *testing.T) {
	...
	for _, tc := range tcs {
		c := NewScopedDistinctString(tc.maxBytes, tc.maxItemsPerScope)
...
func TestScopedDistinctDiff(t *testing.T) {
	c := NewScopedDistinctStringWithDiff(0, 0)
	...
	col := NewScopedDistinctString(0, 0)
...
func TestScopedDistinctStringCollectorIsSafe(t *testing.T) {
	d := NewScopedDistinctString(0, 0) // no limit
...
func BenchmarkScopedDistinctStringCollect(b *testing.B) {
	...
	for _, lim := range limits {
		b.Run("uniques_limit:"+strconv.Itoa(lim), func(b *testing.B) {
			for n := 0; n < b.N; n++ {
				scopedDistinctStrings := NewScopedDistinctString(lim, 0)
