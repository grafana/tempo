...
type KafkaConfig struct {
	...
	// Set to 0 to disable coordinator waiting entirely. Useful for test environments.
	WaitForCoordinatorMinPartitions int `yaml:"wait_for_coordinator_min_partitions"`
...
func (cfg *KafkaConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.WaitForCoordinatorMinPartitions, prefix+".wait-for-coordinator-min-partitions", 500, "Minimum partition count required to wait for Kafka group coordinator readiness after topic creation or partition updates. Set to 0 to disable coordinator waiting (useful for test environments). Only applies when partition count reaches or exceeds this threshold.")
...
// the partition count meets the configured minimum threshold.
func (cfg KafkaConfig) waitForCoordinatorAfterUpdate(ctx context.Context, adm *kadm.Client, logger log.Logger, previousPartitionCount int) error {
	if cfg.WaitForCoordinatorMinPartitions <= 0 || cfg.AutoCreateTopicDefaultPartitions < cfg.WaitForCoordinatorMinPartitions {
		level.Info(logger).Log(
			"msg", "skipping coordinator wait for partition update",
			"topic", cfg.Topic,
			"previous_partitions", previousPartitionCount,
			"new_partitions", cfg.AutoCreateTopicDefaultPartitions,
			"min_partitions_threshold", cfg.WaitForCoordinatorMinPartitions,
		)
		return nil
	}
	return cfg.waitForCoordinator(ctx, adm, logger)
