...
func InitLogger(cfg *server.Config) {
	...
	Logger = kitlog.With(l, "caller", kitlog.Caller(3))
	...
	cfg.Log = logging.GoKit(kitlog.With(l, "caller", kitlog.Caller(4)))
}
...
func NewPrometheusLogger(l logging.Level, format logging.Format) (kitlog.Logger, error) {
...
func CheckFatal(location string, err error) {
	if err != nil {
		...
		if location != "" {
			logger = kitlog.With(logger, "msg", "error "+location)
		}
