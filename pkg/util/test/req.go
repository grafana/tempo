...
func MakeTraceWithTimeRange(requests int, traceID []byte, startTime, endTime uint64) *tempopb.Trace {
	...
	for i := 0; i < requests; i++ {
		...
		trace.ResourceSpans = append(trace.ResourceSpans, makeBatchWithTimeRange(rand.Int()%20+1, traceID, timeRange)) // nolint:gosec // G404: Use of weak random number generator
	}
