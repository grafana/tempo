...
func TestTraceIDToHexString(t *testing.T) {
	tc := []struct {
		...
		{
			byteID:  []byte{}, // empty -> empty
			traceID: "",
		},
		{
			byteID:  nil, // nil -> empty
			traceID: "",
		},
		{
			byteID:  make([]byte, 16), // all zeros -> empty
			traceID: "",
		},
		{
			byteID:  []byte{0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef}, // 8-byte span-sized fast path
			traceID: "1234567890abcdef",
		},
		{
			// 17 bytes: forces the slow path (> 32 hex chars). 0xff + 15 zeros + 0x01.
			byteID:  []byte{0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01},
			traceID: "ff00000000000000000000000000000001",
		},
		{
			byteID:  []byte{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
			traceID: "ffffffffffffffffffffffffffffffff", // no trim
		},
	}
